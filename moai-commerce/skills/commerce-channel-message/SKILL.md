...
5. `moai-core:ai-slop-reviewer` 자동 체이닝
...
푸시 기획 후속: 발송 직전 `moai-commerce:commerce-marketing-compliance-kr`로 정통망법 게이트(야간 21:00-08:00 광고 발송 금지·수신거부·발신자 정보)를 통과시킵니다.
...
| **쿠팡** | 검색영역 + 비검색영역 동시 | 즉시 구매 | 검색(전환 메시지) vs 비검색(노출 메시지) 분리 | CPM 167배 차이 → 영역별 메시지 다르게. moai-commerce:marketplace-coupang-ads와 연계 |
