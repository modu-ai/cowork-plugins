...
> 한국 이커머스 풀세트 자동화 플러그인 — 상세페이지 카피·이미지·사진 + 채널 가이드 8종 + 통합 마케팅 전략·카피·라이브 커머스 + **식약처 안전(MFDS)** (v2.0.0 신규)

## 개요

`moai-commerce`는 한국 이커머스 사업자를 위한 풀 사이클 자동화 플러그인입니다.
상품 사진과 핵심 정보를 제공하면 **13섹션 감정여정 상세페이지**부터 **채널별 입점·운영 가이드**, **마케팅 전략·카피**, **라이브 커머스 진행 스크립트**까지 한 번에 산출합니다. **v2.0.0부터** `mfds-safety`로 헬스/F&B 커머스 상품의 식약처(MFDS) 의약품·식품 안전 정보(e약은요·건강기능식품 인정현황·검사부적합·회수)를 통합 조회합니다(증상 인터뷰 우선, red flag 발견 시 119·응급실 안내 우선).
