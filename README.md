...
두 데스크톱 앱 모두 같은 방식입니다 — 앱의 **설정(Settings) 또는 플러그인(Plugins) 메뉴 → 마켓플레이스(Marketplace)**에서 **+** 로 저장소를 추가하고 주소 `modu-ai/moai-cowork`를 입력합니다. 동기화가 끝나면 직원(코워커) 목록이 표시됩니다.
...
등록한 마켓플레이스에서 필요한 직원을 각각 **+** 버튼으로 추가합니다.
