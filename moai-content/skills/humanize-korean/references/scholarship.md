...
> **외부 SSOT** — 본진 분류 체계(`references/ai-tell-taxonomy.md`)는 패턴 행마다 한 줄 메타(`source_anchor`)로 이 파일을 가리킨다. 학술 출처 전문(full text)은 본 파일에 보존하여 SSOT 룰북의 슬림성을 해치지 않는다.
>
...
> 보고서 §III.3 "8대 번역투 유형의 통합" 매핑. 본진 SSOT 패턴 ID는 v2.0 신규 4건(A-16·A-18·A-19·E-7; A-17은 hold) + 보강 4건(A-15·A-7·F-4·E-2)에 부착 예정.
...
> 보고서 §III.3.4 (line 193-225). 본진 매핑 — 신규 A-17 (gap §2 후보 1순위, none). **v2.0 hold — 외부 회차 양성 0건, NMT 원본 회차 후 v2.1 재평가.**
...
> 보고서 §III.3.8 (line 295-322). 본진 매핑 — E-2(동일 종결어미 반복), G-1(추측·관측형 종결), I-1(것이다 종결), 보강 (gap §3.4·§3.5). 청자 경어법은 본진 미커버 단독 영역 → v2.0 신규 E-7.
...
> 보고서 §V.5.1 (line 388-406) 15항목을 본진 분류 ID + 8유형 anchor에 매핑. 처방 적용은 `rewriting-playbook.md` §1.X 참조 (분류 vs 처방 분리 원칙).
...
> 본진 직접 채택은 caveat C3에 따라 hold (gap §4.1). v2.0 별도 메트릭 트랙(`metrics_v2.py`)에서 정량 지표로 운영.
...
**분류학자 함의**: T8 종결어미 재현율 임계치를 보고서 정량 수치로 못 박을 수 없음. 김혜영 PDF 원문 확보 전까지 'estimated' 플래그 유지(E-7).
...
**분류학자 함의**: post-editese 3축(simplification·normalisation·interference)을 v2.0 분류 체계에 직접 채택할 때, 한국어 정량 검증 부재를 'speculative: true' 플래그로 명기. `baseline_v2.json`의 모든 셀은 `_placeholder: true`.
...
**출처**: [epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai) v2.0.0 (MIT License) `references/scholarship.md`. 한국어 번역투 종합 연구보고서 (2026-05-07) verbatim 보존. valid as of 2026-05.
