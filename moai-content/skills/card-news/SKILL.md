---
name: card-news
description: >
  인스타그램 카드뉴스와 캐러셀을 만들어주는 스킬입니다. "카드뉴스 만들어줘", "인스타 슬라이드 기획해줘",
  "캐러셀 콘텐츠 구성해줘"처럼 말하면 됩니다. AI 이미지 생성 기반 캐러셀 카드뉴스 제작,
  잡지 SOP 적용, 디자인 가이드, AI 글쓰기 방지 기법을 지원합니다.
user-invocable: true
version: 2.11.0
---

# 카드뉴스 (Card News)

## 개요

인스타그램 캐러셀 카드뉴스를 기획하고 제작합니다. AI 이미지 생성 기반 비주얼 콘텐츠, 잡지 SOP 적용 편집 디자인, AI 글쓰기 방지 기법으로 오리지널 문체를 구현합니다. 이미지 생성은 `moai-media` 플러그인의 `nano-banana` 스킬에 위임합니다.

## 트리거 키워드

카드뉴스, 캐러셀, 인스타그램 카드, 슬라이드, AI 이미지, 카드 디자인, 인포그래픽 카드

## 실전 카드뉴스 패턴 (한국 시장)

### 1. 1초 시선 고정 첫 카드 4유형

| 유형 | 첫 카드 카피 예시 | 적용 |
|---|---|---|
| 질문형 | "혹시 이런 경험 있나요?" | 공감 페인 |
| 통계 충격형 | "한국인 78%가 모르는 ~" | 정보 어필 |
| 약속형 | "5분 후, OO를 알게 됩니다" | 가치 예고 |
| 시각 임팩트 | 색·이미지로 시선 잡기 | 비주얼 강함 |

### 2. 콘텐츠 재미 구조 (긴장 → 완화 → 임팩트)

```
1~2장: 긴장 — 문제·페인·질문 제시
3~7장: 완화 — 설명·분석·해결책
8~9장: 임팩트 — "와!" 모먼트 (정량·반전)
10장:  CTA — 단일 액션
```

### 3. 한 카드 한 메시지 원칙

- 한 카드 = 한 헤드라인 + 한 단락 (50자 이내)
- 이미지 + 텍스트 분리 (둘 다 강조 시 충돌)
- 글자 크기 ≥ 24pt (모바일 가독)
- 여백 30%+ (꽉 차면 못 읽음)

### 4. 한국어 가독성 가이드

| 항목 | 권장 |
|---|---|
| 문장 길이 | 한 줄 24자 이내 (모바일) |
| 단어 선택 | 한자어 ↓ 우리말 ↑ |
| 줄간격 | 1.5배 이상 |
| 색 대비 | WCAG AA (4.5:1) |
| 폰트 | Pretendard·Noto Sans KR |
| 강조 | Bold > 색 > 크기 순 |

### 5. 마지막 카드 CTA 6유형

| CTA | 적용 | 효과 |
|---|---|---|
| 저장 | 다시 보고 싶은 정보형 | 알고리즘 ↑ |
| 공유 | 친구 추천형 | 도달 ↑ |
| 팔로우 | 후속 콘텐츠 약속 | 구독 ↑ |
| 링크 | 외부 전환 | 클릭 ↑ |
| 댓글 | 토론·참여형 | 참여 ↑ |
| DM | 1:1 상담·문의 | 리드 ↑ |

1 카드뉴스 = 1 CTA만. 선택지 주면 혼란.

## 워크플로우

### 1단계: 목적 및 주제 파악
- 카드뉴스 목적: 정보 전달 / 브랜드 인지 / 제품 소개 / 리드 수집
- 주제와 핵심 메시지 1가지 확정
- 타겟 독자 정의

### 2단계: 카드 구성 설계 (표준 10장)
```
1장: 표지 (Cover) — 제목 + 후킹 문구 (1초 내 시선 고정)
2~8장: 본문 (Body) — 핵심 정보 1가지/장, 텍스트 최소화
9장: 요약 (Summary) — 핵심 내용 3가지 불릿
10장: CTA — "저장하기", "팔로우", "링크 클릭" 중 1가지만
```

### 3단계: 카드별 카피 작성
- 장당 텍스트: 15-30자 이내 (모바일 가독성)
- 숫자·통계 강조: "3가지", "10분", "87%" 등 시각적 강조
- AI 문체 방지: 나열식 설명 금지, 구어체·직접 화법 사용

### 4단계: 디자인 가이드
```
레이아웃: 9:16 세로 (인스타 최적화) 또는 1:1 정방형
폰트: 제목 굵게(Bold), 본문 Regular, 최대 2가지 폰트
색상: 브랜드 컬러 + 1가지 포인트 컬러 (과다 사용 금지)
여백: 콘텐츠 면적의 30% 이상 여백 확보 (숨 쉬는 디자인)
이미지: 전체 배경 또는 우측 50% 배치
```

### 5단계: 이미지 생성 (AI)

**API 키 필수 (Gemini API):**
```
IF GEMINI_API_KEY 미설정:
  "AI 이미지 생성을 위해 Gemini API 키가 필요합니다.

   발급 방법:
   1. https://ai.google.dev/ 접속 → Google 계정 로그인
   2. API 키 생성 (Pay-as-you-go 유료 플랜 필수)

   API 키를 입력해 주세요:"
```

**모델/해상도 선택:**
- Nano Banana Pro (gemini-3-pro-image-preview): 2K, 텍스트 렌더링 SOTA, 권장
- Nano Banana 2 (gemini-3.1-flash-image-preview): 1K, 빠름, 저비용

**지원 해상도:**
| 해상도 | 크기 | 토큰 비용 | 용도 |
|--------|------|----------|------|
| 512 | 약 0.25MP | 낮음 | 미리보기, 썸네일 |
| 1K | 약 1MP | 1120 토큰 | 기본 (웹용) |
| 2K | 약 4MP | 1120 토큰 | 고해상도 (인쇄) |
| 4K | 약 16MP | 2000 토큰 | 최고 해상도 (프리미엄) |

### 6단계: 산출물 전달
- 카드별 텍스트 + 디자인 가이드
- 이미지 생성 프롬프트 (AI 도구용)
- 인스타그램 캡션 + 해시태그 세트

## 사용 예시

- "스타트업 투자 유치 방법을 카드뉴스로 만들어줘"
- "인스타 캐러셀 10장 기획해줘 (주제: 시간 관리)"
- "제품 소개 슬라이드 콘텐츠 구성해줘"
- "AI 이미지로 카드뉴스 만들어줘"
- "잡지 스타일 인포그래픽 카드 만들어줘"

## 출력 형식

- 카드별 텍스트 (15-30자, 장당 핵심 메시지)
- 디자인 가이드 (레이아웃, 폰트, 색상, 여백)
- AI 이미지 생성 프롬프트 (Gemini 3 기반)
- 인스타그램 캡션 + 해시태그 세트

## 주의사항

⚠️ **AI 생성 콘텐츠 주의**: AI가 생성한 카드뉴스 내용은 사실 확인 후 게시하세요. 통계, 수치, 인물 정보는 반드시 원출처를 검증하세요.

**이미지 생성 실패**: Python 스크립트 실행이 불가한 환경에서는 이미지 생성 프롬프트만 제공하고 Midjourney, DALL-E, Adobe Firefly 등에서 직접 생성하세요.

**텍스트 분량 초과**: 카드 1장당 30자 초과 시 핵심 키워드만 남기고 나머지는 이미지 설명이나 캡션으로 이동하세요.

**플랫폼 규격 변경**: 인스타그램 캐러셀 규격은 변경될 수 있습니다. 최신 규격은 인스타그램 공식 채널에서 확인하세요.

## 관련 스킬

- `moai-media:nano-banana` — Gemini 3 기반 AI 이미지 생성
- `moai-content:blog` — SEO 블로그 포스팅 작성
- `moai-content:copywriting` — 마케팅 카피·헤드라인 작성
- `moai-core:ai-slop-reviewer` — AI 생성 텍스트 패턴 검수

## 이미지 생성 연동

이미지 생성은 **`moai-media` 플러그인의 `nano-banana` 스킬**이 담당합니다. card-news는 프롬프트만 생성하고 실제 렌더링은 moai-media에 위임합니다.

모델/해상도/비율 미지정 시 사용자에게 선택 요청:
1. 모델: Nano Banana Pro (권장) / Nano Banana 2
2. 해상도: 1K (권장) / 2K / 4K / 512
3. 비율: 3:4 (인스타 피드, 권장) / 1:1 / 9:16 / 16:9