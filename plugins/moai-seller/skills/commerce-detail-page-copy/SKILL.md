...
description: >
  ...
  페어 moai-seller:commerce-product-detail과 명확히 구분 — 본 스킬은 카피, 페어는 코드 (shadcn/ui React 또는 HTML).
  체이닝 권장: commerce-detail-page-copy(카피) → commerce-product-detail(코드) 또는 commerce-detail-page-copy → commerce-detail-page-image(1080×12720 합성 PNG).
...
체이닝: `commerce-jtbd-persona --mode persona` → **commerce-detail-page-copy --mode diagnose** → **commerce-detail-page-copy --mode copy** → `commerce-product-naming`

- `commerce-jtbd-persona` — JTBD·페르소나 (--mode copy 입력)
- `moai-seller:commerce-detail-page-image` — 생성된 카피를 기반으로 13섹션 이미지 합성
- `moai-seller:commerce-marketplace-coupang` — 쿠팡 정책 적용 최적화
...
- 랜딩 페이지(마케팅 원페이지): `moai-marketer:marketing-landing-page` 사용
- 블로그 포스팅: `moai-marketer:content-blog` 사용
- SNS 콘텐츠: `moai-marketer:content-sns-content` 사용
- 이미지 합성만 필요할 때: `moai-seller:commerce-detail-page-image` 직접 사용
