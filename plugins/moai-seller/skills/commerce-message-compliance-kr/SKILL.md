...
description: |
  ...
  광고·정보성 메시지 자동 판정 + 야간 발송 차단 + 표기 위치 검증 + 무료 수신거부 명시 점검 전담. 모든 채널(SMS/LMS/MMS·이메일·앱푸시·카톡 친구톡/알림톡·텔레마케팅) 발송 전 의무 통과 법규 게이트. 페어 스킬 commerce-channel-message(메시지 카피 작성)와 명확히 구분 — 본 스킬은 발송 전 사전 점검, 페어는 카피 생성.
  ...
  general-ai-slop-reviewer 체이닝 제외 (규칙 기반 검증 스킬).
version: "1.0.0"
---
...
체이닝 순서: `commerce-channel-message` (카피 작성) → **commerce-message-compliance-kr** (법규 게이트) → 실제 발송
