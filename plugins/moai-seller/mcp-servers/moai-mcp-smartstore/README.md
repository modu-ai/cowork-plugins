...
9개 도메인(~140 엔드포인트)을 **90개 MCP 도구**로 래핑. 전체 도구 목록은 `manifest.json` 또는 `moai-mcp-smartstore` 실행 후 `tools/list` 로 확인.
...
# 버전 확인
uvx moai-mcp-smartstore --version
...
플러그인 vendor 모델 — `uv run --directory ./mcp-servers/moai-mcp-smartstore moai-mcp-smartstore` 로 end-user 기동 시 `.venv` 가 자동 생성된다(gitignored).
