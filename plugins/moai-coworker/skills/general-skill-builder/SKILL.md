...
스킬 생성 스킬 제작 general-skill-builder 새 스킬 만들기 스킬 추가 신규 스킬 harness 워크플로우
...
Phase 3: Skill Draft    → general-skill-template 기반 SKILL.md 초안 + ## 출처 섹션
Phase 4: Test Gen       → 테스트 프롬프트 2-3개 + 기대 출력 정의
Phase 5: Validation     → 4차원 루브릭 스코어링 (general-skill-tester 본문 §스코어링 루브릭)
...
`general-skill-template` 템플릿을 기반으로 SKILL.md 초안을 생성합니다.

**필수 적용:**

1. general-skill-template의 필수 섹션 구조 사용
...
신규 스킬에 한국어 예시 카피를 넣을 때 AI 슬롭 패턴이 예시로 전염되지 않도록 아래 3종을 금지합니다. 이 규칙은 `general-ai-slop-reviewer`·`general-humanize-korean` 게이트가 슬롭으로 분류하는 패턴이 "좋은 예"로 등록되는 것을 원천 차단합니다.
...
| 스킬 | 관계 | 설명 |
|------|------|------|
| general-skill-template | before | 템플릿 구조를 제공하는 기반 스킬 |
| general-skill-tester | after | 생성된 스킬의 테스트 실행 + 루브릭 스코어링 |
| general-ai-slop-reviewer | after | 스킬 본문의 AI 패턴 검수 (선택) |

## 관련 커맨드

| 커맨드 | 설명 |
|--------|------|
| `/harness` | new→test→review 자동 연쇄 (이 스킬 + general-skill-tester + general-ai-slop-reviewer 오케스트레이션) |
...
- **1.5.x**: skill-forge → general-skill-builder 이름 변경
