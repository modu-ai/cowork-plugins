...
meta-skill-builder로 생성된 스킬 또는 기존 스킬의 품질을 검증합니다. harness의 A/B 테스트 방법론을 구현하여 baseline(스킬 없음)과 with-skill(스킬 사용)을 비교하며, **4차원 스코어링 루브릭**과 **체인 검증 프로토콜**을 본문에 직접 포함하여 별도 rules 파일 참조 없이 단독으로 동작합니다.
...
| Chain | Steps | Status |
|-------|-------|--------|
| 사업계획서(PPT) | consult-strategy → doc-pptx → ai-slop-reviewer | 검증 대상 |
| 블로그 발행 | content-blog → ai-slop-reviewer → (optional) media-higgsfield-image | 검증 대상 |
| 제품 랜딩 | content-copywriting → marketing-landing-page → ai-slop-reviewer | 검증 대상 |
