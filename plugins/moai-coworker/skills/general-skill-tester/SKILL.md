...
description: |
  ...
  - "스킬 테스트해줘", "스킬 검증", "general-skill-tester"
...
general-skill-builder로 생성된 스킬 또는 기존 스킬의 품질을 검증합니다. harness의 A/B 테스트 방법론을 구현하여 baseline(스킬 없음)과 with-skill(스킬 사용)을 비교하며, **4차원 스코어링 루브릭**과 **체인 검증 프로토콜**을 본문에 직접 포함하여 별도 rules 파일 참조 없이 단독으로 동작합니다.

## 트리거 키워드

스킬 테스트 검증 A/B 테스트 baseline 회귀 테스트 체인 테스트 general-skill-tester 품질 측정 루브릭 스코어링 4차원 평가
...
general-skill-builder Phase 5에서 사용하는 기본 모드입니다.
...
| 스킬 | 관계 | 설명 |
|------|------|------|
| general-skill-builder | before | 스킬 생성 후 테스트 실행 |
| general-skill-template | before | 템플릿 기반 스킬 구조 정의 |
| general-ai-slop-reviewer | alternative | 텍스트 품질 검수 (비기능적) |
