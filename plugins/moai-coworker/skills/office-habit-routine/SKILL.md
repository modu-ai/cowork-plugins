...
- "며칠 연속" 카운트가 동기를 만든다
- → 노션·달력 등에 트래커 구성 → `office-notion-template-kit` 연계
...
- → 끊긴 지점을 `office-retro-builder`로 가볍게 점검하고 문턱을 더 낮춘다
...
- **moai-officer:office-time-system**: 습관을 시간 블록에 배치해 실행 보장
...
- **팀 업무 프로세스·체크리스트 표준화**: 조직 루틴은 `moai-coworker`를 사용한다.
- **목표가 아직 없는 경우**: 무엇을 습관화할지 모르면 `office-goal-planner`가 먼저다.
