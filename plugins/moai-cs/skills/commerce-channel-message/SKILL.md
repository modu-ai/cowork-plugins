...
| **쿠팡** | 검색영역 + 비검색영역 동시 | 즉시 구매 | 검색(전환 메시지) vs 비검색(노출 메시지) 분리 | CPM 167배 차이 → 영역별 메시지 다르게. moai-seller:commerce-marketplace-coupang-ads와 연계 |

### 인지 편향 9종 적용 가이드

페어 스킬 `moai-marketer:marketing-campaign-planner`와 동일한 9종 (Kahneman 행동경제학 표준 카탈로그 기반).
...
| 무신사 | 앱푸시 | "사이즈 마지막 1개, 지금 바로" | 희소성·손실 회피 |
