...
`moai-tutor:education-learning-material`이 생성하는 HTML 학습자료에서 사용하는 CDN 라이브러리의 **단일 진실(SSOT)**. 영역별 최고 수준 라이브러리를 선정하고, CDN URL·초기화 스니펫·조건부 로딩 규칙을 정의한다.
...
4. **라이선스 안전** — CDN 런타임 로딩은 저장소 Apache-2.0 라이선스와 무관하다(재배포가 아니라 사용자 브라우저가 직접 로드). 각 라이브러리의 OSS 라이선스(MIT/Apache-2.0/BSD)는 런타임 사용에 제약이 없다.
