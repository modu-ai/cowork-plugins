...
description: |
  ...
  [책임 경계] vs moai-tutor:education-curriculum-designer: 이 스킬=배우는 사람 본인의 자기주도 학습 프로젝트, 저 스킬=강사가 남을 가르치려고 만드는 강의 커리큘럼.
version: "1.0.0"
---
...
학습자가 스스로 공부할 **개인 학습 프로젝트**를 초기화한다. 학습 주제와 현재 수준을 진단하고, 도달 목표까지의 단계별 로드맵(마일스톤)을 설계하며, 진도 추적 파일과 학습 전용 `CLAUDE.md`를 생성한다. 이후 학습 중 궁금한 점은 `education-tutor-research`로 조사하고, 정리된 자료는 `education-learning-material`로 만드는 사이클의 출발점이다.
...
각 마일스톤마다: 학습 목표 한 줄, 핵심 질문 2-3개(→ `education-tutor-research` 입력), 완료 기준(자가 점검), 예상 학습 분량을 명시한다. **소요 시간 단정은 피하고** 우선순위와 순서로 표현한다.
...
└── materials/          # education-learning-material 산출 HTML 보관
```
...
## 학습 워크플로우 (HARD)
1. 막히는 개념은 `moai-tutor:education-tutor-research`로 최신 정보 병렬 조사
2. 조사 결과는 `moai-tutor:education-learning-material`로 도식·예제 HTML 학습자료 생성
...
- 첫 마일스톤의 핵심 질문으로 `education-tutor-research`를 실행하라는 가이드
...
→ 목표 구체화 → 가정법 유형별 마일스톤 → 진도 시트 → education-tutor-research 연결
...
- **moai-tutor:education-tutor-research**: 마일스톤 핵심 질문을 최신 정보로 병렬 조사
- **moai-tutor:education-learning-material**: 조사 결과를 도식·예제 HTML 학습자료로 생성

## 이 스킬을 사용하지 말아야 할 때

- **남을 가르치는 강의 커리큘럼 설계** → `moai-tutor:education-curriculum-designer`
- **시험·평가 문제 출제** → `moai-tutor:education-assessment-creator`
- **학술 논문·문헌 검토** → `moai-tutor:education-research-assistant`
