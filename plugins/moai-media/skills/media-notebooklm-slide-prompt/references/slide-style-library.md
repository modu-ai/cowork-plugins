...
본 라이브러리는 공개된 옵시디언 publish 자료(이커머스 클래스 · 노트북 LM 슬라이드 가이드북)에서 49개 시각 스타일 분류를 참고해, MoAI 모두의 코워크에서 사용하기 위해 카테고리·매칭 규칙·일관성 가드를 재구성한 자료입니다. 영문 프롬프트는 Nano Banana(Gemini 3 Pro Image) 모델 직접 인식을 위해 의도적으로 짧은 키워드 조합 형태를 유지했습니다.
