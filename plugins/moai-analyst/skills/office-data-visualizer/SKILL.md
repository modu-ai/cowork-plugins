...
2. 데이터 테이블 + 차트 이미지 → `moai-officer:office-pptx-designer`로 전달
3. PPT 슬라이드 생성

Word 보고서 요청 시:
1. 차트 이미지 + 분석 텍스트 → `moai-officer:office-docx-generator`로 전달
2. Word 문서 생성 (차트 임베드)
...
3. 데이터 구조 확인 → 부적합 시 `moai-analyst:office-data-explorer` 권유
4. 차트 산출 후 사용자 검토 요청

## 이 스킬을 사용하지 말아야 할 때
- **데이터 탐색/프로파일링** → `moai-analyst:office-data-explorer` 사용
