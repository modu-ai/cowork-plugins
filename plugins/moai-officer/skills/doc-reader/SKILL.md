...
description: |
  ...
  본 스킬은 "읽기(파싱)" 전담입니다. 문서 "생성(쓰기)"은 doc-pdf·doc-hwp·doc-docx·doc-xlsx를 사용하세요.
version: "1.1.0"
---
...
| 작업 | 스킬 | 비고 |
| --- | --- | --- |
| HWP/HWPX 문서 **생성**(기안서·품의서·공문) | `moai-officer:doc-hwp` | 한컴오피스 호환 .hwpx 파일 작성 |
| 워드(.docx) 문서 **생성** | `moai-officer:doc-docx` | 보고서·계약서·제안서 |
| PDF **생성** | `moai-officer:doc-pdf` | weasyprint 엔진, CJK 폰트 임베딩 |
| 엑셀(.xlsx) **생성** | `moai-officer:doc-xlsx` | KPI 대시보드·매출 분석표 |
| HWP/HWPX/PDF **읽기·파싱** | **`moai-officer:doc-reader` (본 스킬)** | kordoc MCP |

예외: `fill_form`은 기존 양식 템플릿의 빈칸을 채우는 동작이므로 본 스킬이 담당합니다(새 문서를 생성하지 않음).

## 이 스킬을 사용하지 말아야 할 때

- **새 문서를 처음부터 작성** → `moai-officer:doc-pdf` / `doc-hwp` / `doc-docx` / `doc-xlsx`
- **HTML·Markdown을 PDF로 변환** → `moai-officer:doc-pdf` (weasyprint 엔진)
...
- **after**: `moai-analyst:data-explorer` — 파싱된 표 데이터 분석
- **after**: `moai-officer:doc-pdf` — 파싱된 내용으로 새 PDF 생성
- **after**: `moai-officer:doc-hwp` — 파싱된 내용으로 새 한글 문서 생성
- **after**: `moai-officer:doc-docx` — 파싱된 내용으로 새 워드 문서 생성
- **after**: `moai-analyst:data-visualizer` — 파싱된 표 데이터 차트 시각화
- **pair**: `moai-officer:setup-mcp-connector` — kordoc MCP 사전 준비 가이드
