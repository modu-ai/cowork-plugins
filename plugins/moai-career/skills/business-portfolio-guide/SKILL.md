...
```
moai-career:business-portfolio-guide → moai-coworker:general-ai-slop-reviewer → moai-writer:general-humanize-korean
```
...
지원 서류는 `moai-career:business-resume-builder`, JD 분석은 `moai-recruiter:business-job-analyzer`, 면접 준비는 `moai-career:business-interview-coach`와 함께 사용하세요.

> 표·스크린샷·다이어그램·정량 대시보드 같은 비서술 산출물은 이 체인 대상이 아닙니다. 서술 텍스트 부분에만 적용합니다.

## 이 스킬을 사용하지 말아야 할 때

- **자소서/이력서 작성**: 지원 서류 작성은 `moai-career:business-resume-builder` 스킬을 사용하세요.
- **면접 준비**: 포트폴리오 발표 면접이 아닌 일반 면접은 `moai-career:business-interview-coach` 스킬이 적합합니다.
- **채용공고 분석**: JD 분석과 역량 매칭은 `moai-recruiter:business-job-analyzer` 스킬을 사용하세요.
