...
| AI 슬롭 후처리 + 최종 검수 블록 | 약 12라인 | `moai-coworker:ai-slop-reviewer` → `moai-writer:korean-humanize` → 최종 검수 규칙(HARD 고정). 최종 검수는 생략 불가 |
...
**개선 경로 예산 재검증**: 자가 개선 diff가 `AGENTS.md`를 수정한 경우에도 §5-4 길이 검증(`wc -l` ≤ 200)을 재실행한다. 라인 예산은 생성 시 1회 검증이 아니라 **`AGENTS.md`가 수정될 때마다 지켜야 하는 불변식**이며, 초과 시 축소 대상은 생성 시와 동일하다(체인 나열만 — 8개 HARD 블록은 축소 대상이 아니다).
