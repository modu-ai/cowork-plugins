...
**스킬 체인 정의 유효성**: `AGENTS.md` "프로젝트 워크플로우"의 각 체인이 설치된 플러그인에 실재하는지 검증하고, 한국어 텍스트 산출물 체인 말미에 ⟨한국어 감사 3단⟩(`moai-coworker:ai-slop-reviewer` → `moai-writer:korean-spell-check` → `moai-writer:korean-humanize`) **세 단계가 이 순서대로** 있는지 확인한다. `ai-slop-reviewer` 하나만 있으면 **결함**이다 — 정본은 `cowork-setup.md` §3.
