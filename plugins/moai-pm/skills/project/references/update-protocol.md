...
# 소스 C: 현재 세션 system reminder의 "user-invocable skills" 목록(moai-* 접두만)
```
...
`new_inventory`를 `.moai/config.json`의 스냅샷과 비교해 세 가지 diff를 만든다:
...
**[HARD] 스킬 diff가 비어도 종료하지 않는다 — 템플릿 drift를 따로 본다.** 두 축은 독립이다.

| 축 | 무엇을 비교 | 비면 |
|---|---|---|
| **스킬 diff** | `.moai/config.json`의 `plugins_installed` 버전 + `skills_available` digest ↔ 현재 설치 | 이 축만 건너뛴다 |
| **템플릿 drift** | `config.json`의 `template_version`·`hard_block_digests` ↔ `references/templates/AGENTS.md.tmpl` 현재 값 | 이 축만 건너뛴다 |

**둘 다 비었을 때만 종료한다.** 스킬이 하나도 안 바뀌어도 템플릿 HARD 블록이 개선됐으면 §4-1 재동기화는 반드시 돌아야 한다 — 그러지 않으면 정본 개선이 기존 폴더에 영원히 도달하지 못한다(실제 사례: 2026-08-27 §6 한국어 품질 체인 재작성).

## 3. 세션 신호 분석 (재귀적 자가 학습 입력)

"기존 대화 세션을 분석"하는 실체 절차다. 트랜스크립트 전체를 뒤지는 게 아니라 **영속화된 신호 파일**을 읽는다 — 세션이 바뀌어도 유실되지 않는다.

1. **`.moai/evolution/signals.md` 읽기** — 누적된 `repeated correction`·`chain failure` 신호를 최근 순으로 스캔.
2. **`.moai/context.md` 읽기** — 프로젝트 맥락(목적·주요 산출물·톤·금지 사항) 재확보.
...
diff에 맞춰 프로젝트 산출물을 갱신한다. **사용자가 쓴 구획은 전면 재작성 금지**(최소 diff 원칙, SKILL.md §Recursive Self-Improvement). **템플릿이 소유한 HARD 블록은 최신본으로 재동기화한다** — 아래 4-1 소유권 표가 그 경계다.

### 4-1. AGENTS.md

**소유권을 나눠서 다룬다.** 한 파일 안에 성격이 다른 두 종류가 섞여 있고, 갱신 방식도 달라야 한다.

| 구획 | 소유 | 갱신 방식 |
|---|---|---|
| **8개 `## N. … (HARD)` 블록** | 템플릿 (`references/templates/AGENTS.md.tmpl`) | **템플릿 최신본으로 재동기화** |
| 프로젝트 개요·워크플로우 표·라우팅·커넥터·맥락 | 사용자/프로젝트 | **최소 diff** (전면 재작성 금지) |

- **[HARD] 3-way merge로 판정한다 — 「다르면 덮어쓴다」로는 구분이 안 된다.** base는 `config.json`의 `hard_block_digests`가 가리키는 **이전 템플릿 블록**이다.

  | base ↔ 현재 문서 | base ↔ 새 템플릿 | 처리 |
  |---|---|---|
  | 같다 (사용자 미수정) | 다르다 | **덮어쓴다** — 조용히 적용 |
  | 다르다 (사용자 수정) | 다르다 | `AskUserQuestion`으로 확인. 원문을 `.moai/evolution/`에 남긴다 |
  | 다르다 | 같다 | 사용자 수정 존중 — 손대지 않는다 |

  `hard_block_digests`가 없는 구 프로젝트는 base를 모르므로 **전부 「사용자 수정」으로 간주**해 확인을 받는다. 모르면 묻는 쪽이 기본이다.
- **[HARD] 렌더 후 원자적 교체.** 치환 결과를 임시 파일에 쓰고 `grep -nE '\{[a-z_]+\}'`가 **0건**인지 확인한 뒤에만 실제 파일로 옮긴다. 미치환 슬롯이 남은 문서를 사용자에게 남기지 않는다.
- **[HARD] HARD 블록 재동기화 — 최소 diff의 예외다.** 정본이 개선돼도 기존 폴더가 못 받으면 개선이 전파되지 않는다. 템플릿의 8개 HARD 블록을 현재 `AGENTS.md`의 같은 블록과 대조해, **다르면 템플릿 쪽으로 덮어쓴다.** 사용자가 HARD 블록을 직접 손댔다면 덮어쓰기 전에 `AskUserQuestion`으로 확인하고, 원문 조각을 `.moai/evolution/`에 남긴다.
  - 실제 사례(2026-08-27): 템플릿 §6이 「AI 슬롭 후처리 2단」에서 「한국어 품질 체인 3단 + 작성 기준」으로 바뀌었다. 최소 diff만 돌면 기존 폴더는 **영원히 2단에 머문다.**
- **[HARD] 재동기화 후 변수 슬롯을 다시 채운다.** 템플릿 블록에 `{변수}`가 있으면 `.moai/context.md`와 기존 `AGENTS.md`에서 값을 회수해 치환한다. 치환 못 한 슬롯이 남으면 생성을 멈추고 보고한다 — 사용자 문서에 `{audience}` 같은 날것이 남는 것이 가장 흔한 사고다.
- **[HARD] 체인 종료 규칙 재검사.** 한국어 텍스트 산출물 행이 `⟨한국어 감사 3단⟩`(`ai-slop-reviewer` → `korean-spell-check` → `korean-humanize`) **순서대로** 끝나는지 확인한다. `ai-slop-reviewer`만 있거나 순서가 뒤집혀 있으면 고친다 — `korean-humanize`가 마지막이 아니면 최종 검수를 통과한 산출물이 전달되지 않는다.
- **워크플로우 표** — 추가된 스킬을 적합한 산출물 행에 편성, 제거된 스킬 행은 삭제 또는 대체 스킬로 교체. **새로 설치된 플러그인의 스킬을 전수 훑어** 기존 체인을 더 잘 만들 수 있는 자리가 있는지 본다(설치만 되고 어느 체인에도 안 쓰이는 스킬은 보고한다).
- **500라인 예산 준수** — 갱신 후 `wc -l`로 재검증. 초과 시 스킬 체인 나열만 축소(HARD 블록 8개는 절대 축소·삭제하지 않는다 — `agentsmd-generator.md` §5).
...
- **실패 판정 시**: `.moai/evolution/`에 남긴 원문 조각으로 §4 diff를 되돌린다.
...
- `agentsmd-generator.md` — AGENTS.md 500라인 예산·8 HARD 블록 보존 정책·CLAUDE.md 포인터 규칙·레거시 마이그레이션(§7.1)
