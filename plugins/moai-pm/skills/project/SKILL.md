...
## Socratic Interview (3-Stage · 심층 필수)

이 스킬은 사용자에게 **이 프로젝트에서 무엇을·어떻게 처리하고 싶은지**만 인터뷰한다. 이름·회사 등 글로벌 프로필은 재질문하지 않는다.

**구조는 3단계로 고정하되, 질문 세트는 고정하지 않는다.** [HARD] **S2 심층 라운드는 조건부가 아니라 필수다** — 프로젝트 맥락을 얕게 잡으면 그 얕음이 폴더 지침·스킬 체인·모든 산출물에 그대로 복제된다. S1 네 질문으로는 산출물별 도메인 제약과 실패 조건을 담을 수 없다. 과거의 「업무유형 → 산출물 → 톤」 3연발 순차 호출과 「텍스트 대화 심화 인터뷰」 분리 방식은 **폐기**한다 — 질문은 항상 `AskUserQuestion` 설문으로, 한 라운드에 **묶어서** 낸다.

| 단계 | 목적 | 호출 | 질문 구성 |
|---|---|---|---|
| **S1 일괄 진단** | 프로젝트 설계에 필요한 맥락을 **한 번에** 확보 | `AskUserQuestion` **1회** | 질문 풀에서 정보 이득이 큰 순으로 **최대 4개를 한 화면에 배치** |
| **S2 심층 (필수)** | 산출물별 도메인 제약·실패 조건·기존 자산을 캔다 | `AskUserQuestion` **최소 1회** | 아래 S2 질문 풀에서 4개 |
| **S3 확인** | 설계안(체인·에이전트)을 보이고 승인받는다 | `AskUserQuestion` **1회** | 설계 요약 + 승인/수정/취소 |
...
3. 이미 A등급으로 확립된 축(진입 발화·기존 `AGENTS.md`·`.moai/context.md`에서 획득)은 **질문에서 제외**하고, 빈 슬롯은 다음 순위 질문으로 채워 **항상 4슬롯을 채운다**.
4. 모든 옵션에 "선택하면 무엇이 달라지는지"를 `description`으로 붙인다(빈 설명 금지). 첫 옵션에만 `(권장)` 라벨을 단다.

**S2 질문 풀** (심층 — 매 실행 4개 선정. 산출물 종류에 따라 축이 달라진다):
⑩ 이 산출물이 **실패하는 조건** — 무엇이 있으면 못 쓰는 결과물인가 · ⑪ 지금 이 일을 **어떻게 하고 있는가**(현행 절차·소요 시간) · ⑫ 참고할 **기존 자산**(과거 산출물·브랜드 자산·양식·용어집) · ⑬ **누가 검토·승인**하는가 · ⑭ 다루는 자료의 **민감도**(계약·개인정보·대외비 여부 — §한국어 감사의 맞춤법 단계 생략 판단에 쓴다) · ⑮ **반복 주기와 분량** · ⑯ 연동해야 할 **외부 서비스·계정** · ⑰ 이 프로젝트에서 **절대 하면 안 되는 것**.

**S2 실행 규칙 (HARD)**:
1. **건너뛰지 않는다.** S1 답이 충분해 보여도 최소 1회 실행한다 — "충분해 보인다"는 판단 자체가 얕은 맥락에서 나온다
2. ⑭ 민감도는 **항상 묻는다.** 이 답이 `korean-spell-check` 외부 전송 여부를 가른다(§6-3)
3. S1에서 이미 A등급으로 확립된 축은 제외하고 다음 순위로 4슬롯을 채운다
4. **[HARD] 첫 S2부터 「지금 아는 것으로 진행」 선택지를 함께 배치한다.** 심층이 필수라고 해서 사용자를 가둬서는 안 된다 — 짧은 요청에도 이탈 경로가 항상 있어야 한다. 다만 **민감도(⑭)만은 이탈해도 반드시 답을 받는다.** 답이 없으면 `unknown`으로 기록하고, `unknown`은 외부 전송을 하지 않는 쪽으로 처리한다(§6-3)
5. 답이 여전히 모호하면 S2를 한 번 더 돌린다
