...
weight: 1
description: "moai-media 정리(16→4) + 강의 컨텍스트 전면 제거 + moai-education/career/bi 재정의 + docs-site 일관성 대대적 정리. 22 플러그인 유지, 155 → 143 스킬. Breaking change 없음."
...
v2.11.0은 **"책임 경계 재정렬 + 강의 컨텍스트 제거 + docs-site 일관성 대대적 정리"** 릴리스입니다.
...
- **강의 컨텍스트 전면 제거** — "3일 마스터 캠프", "Day 1/2/3", "V6 6도구", "21세션·18 산출물" 등 특정 강의 진행 마커가 SKILL.md·docs-site·README·academy handoff 전반에서 제거되어 본 저장소는 도메인 스킬 마켓플레이스 정체성으로 환원됩니다.
- **moai-education / moai-career / moai-bi 재정의** — 각각 강사·교수·교사 풀스택 / 한국 취준생·재직자 2026 채용 / html-report 중심으로 정체성 갱신.
- **docs-site 대대적 정리** — 물결 `~` strikethrough 정정 266+ 파일, mermaid 가로→세로 67 파일, GOAL+AskUserQuestion+스킬 체이닝 공통 골격 적용, 터미널 prompt shortcode 통일, 본문 진행 마커 일괄 제거.
...
- `media-moodboard`, `media-gpt-image2-builder`, `media-model-router`, `media-channel-ad-packager`, `media-ai-disclosure`, `media-canva-magic-layer` — 강의 컨텍스트(Day 3 광고 풀세트)에서 파생된 wrapper. 책임 경계 정리로 제거

### 강의 컨텍스트 완전 제거

특정 강의("모두의 커머스 3일 마스터 캠프")의 진행 마커가 본문에 누적되어 있던 문제를 정정했습니다.

- 제거된 표현: "3일 마스터 캠프", "Day 1/2/3", "V6 6도구", "21세션·18 산출물", "🎓 강의로 배우기" 카드, academy 외부 CTA 다수
- SKILL.md·docs-site `_index.md`·`/plugins/*`·`/cookbook/tracks/*`·README 전반에서 일괄 제거
...
| 플러그인 | 이전 | 재정의 후 |
|---|---|---|
| **moai-education** | "3일 캠프 운영 매뉴얼·30일 후기 자산화" 중심 | "강사·교수·교사 교육 콘텐츠 풀스택" — `course-curriculum-design`은 1일~16주 모든 강의 형식 지원, `course-followup-sequence`는 일반 강의·연수·정규 강좌 후 후기 자산화 |
| **moai-career** | 일반 한국 채용 가이드 | 한국 취준생·재직자 2026 채용 트렌드 반영(팀핏 면접·핀셋 채용·AI 진정성·4 플랫폼 MAU·헤드헌터 5축·NCS·블라인드). vault 인사이트만 참고(PUBLY 원문 0건 그대로) |
| **moai-bi** | 일반 보고서 산출물 | `executive-summary` 산출물을 `html-report` 중심으로 재정의. 단일 HTML 파일에 이미지·CSS·JS 모두 인라인(카톡 즉시 확인 가능) + pdf/docx/pptx/hwpx 변환은 옵션 |

### docs-site 일관성 대대적 정리

- **물결 strikethrough 사고 정정** — 266+ 파일 1,000+ 변환 (`~문구~` 가독성 깨짐 일괄 정정)
- **mermaid 가로(LR) → 세로(TD) 변환** — 67 파일 77 블록, 모바일 가독성 개선
- **GOAL + AskUserQuestion + 스킬 체이닝 공통 골격** — `cookbook/tracks/` 전체 일괄 적용
...
{{< terminal title="claude — cowork" >}}
> 강의 후기 자산화 시퀀스 짜줘. 16주 정규 강좌 후.
{{< /terminal >}}

→ `course-followup-sequence` 호출 → AskUserQuestion(강의 형식: 1일·8주·16주·연수) → D+1~D+30 후기 수집 시퀀스 + 채널별 자산화 가이드.
...
| `media-*` (Day 3 광고 풀세트 wrapper) | 외부 MCP + 프롬프트 빌더 3종 조합 |
...
- **강의(camp) 컨텍스트 참고하던 사용자**: 강의 컨텍스트는 외부 사이트(academy.mo.ai.kr)로만 안내. 본 저장소의 도메인 스킬은 그대로 동작
- **docs-site 즐겨찾기 사용자**: 삭제된 4 페이지(`cookbook/ai-employee-design` · `lab-1` · `lab-2` · `final-project`)는 메뉴에서 제거됨. 다른 페이지는 그대로
