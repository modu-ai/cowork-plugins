...
각 스킬의 `description` 필드에 트리거 조건이 쓰여 있습니다. Cowork가 사용자의 요청을 읽고 일치하는 스킬이 있으면 자동으로 호출합니다. 예를 들어 "블로그 글 써줘"는 `moai-content:blog`를, "계약서 검토해줘"는 `moai-legal:contract-review`를, "사업계획서 만들어줘"는 `moai-business:strategy-planner`를 자동으로 불러옵니다.
...
modu-ai/cowork-plugins 마켓플레이스는 21개 플러그인 안에 144개 스킬을 묶어 제공합니다. 스킬을 직접 만들 필요 없이 자연어 한 줄로 목표를 던지면 시스템이 적합한 스킬을 자동으로 선택하고 체이닝합니다. 콘텐츠·디자인·재무·법무·운영·마케팅·이커머스 등 21개 플러그인의 전체 목록은 [플러그인 카탈로그](../../plugins/)에서, 자주 쓰는 스킬 조합 12종은 [쿡북 — 스킬 체인 설계](../../cookbook/skill-chaining/)에서, 업무 영역별 한 줄 요청과 자동 체인 시나리오는 [쿡북 트랙](../../cookbook/tracks/)에서 확인할 수 있습니다.
