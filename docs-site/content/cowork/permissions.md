...
**위 층이 막히면 아래 층은 자동으로 무력화**됩니다. OS가 차단하면 앱이 폴더를 볼 수 없고, 앱이 폴더를 볼 수 없으면 프로젝트도 새 태스크도 R/W가 막힙니다.
...
권한 다이얼로그를 무심코 "허용 안 함"으로 눌렀다면 이후 시스템 환경설정에서 직접 켜야 합니다. 폴더를 다른 위치로 옮긴 뒤 그대로 작업을 시도하면 경로가 바뀌어 권한이 끊기므로 폴더를 다시 선택해야 합니다. 다른 프로젝트에서 같은 파일을 참조하려면 두 프로젝트 모두 그 폴더에 바인딩되어 있어야 합니다. VPN·SSO·MDM으로 통제되는 회사 PC에서 폴더가 보이지 않는다면 회사 IT가 폴더 접근 권한을 별도로 제한하는 경우이므로 IT 팀에 Claude 앱의 접근 허용을 문의하세요.
