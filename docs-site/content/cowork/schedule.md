...
![채널 및 태스크 관리](/screenshots/getting-started/quick-start-project-7.png)

1. **채널 이름** — 예약 작업을 관리할 채널 이름을 입력합니다
2. **설명** — 채널에 대한 설명을 추가합니다
3. **카테고리 드롭다운** — 작업 카테고리를 선택합니다
4. **공개/비공개 토글** — 채널의 공개 여부를 설정합니다
5. **가이드 텍스트** — 채널 사용에 대한 가이드를 입력합니다
6. **저장 버튼** — 설정을 저장합니다
...
주·월간 보고나 대시보드 갱신처럼 정기·정량적인 작업은 예약으로, NDA 검토나 견적서 초안처럼 품질 검토가 필요한 임시 작업은 디스패치로 지시한 뒤 데스크톱에서 직접 결과를 확인하는 방식이 권장됩니다.
...
디스패치는 **사용자의 계정**을 통해서만 작동합니다. 내 Claude 계정에 로그인된 디바이스에서만 지시할 수 있으며, 외부에서 임의로 데스크톱을 깨우는 것이 아니라 내 계정에 묶인 데스크톱이 작업을 받아 처리하는 방식입니다(전달 메커니즘의 정확한 구현은 Anthropic 정식 문서에 미공개). 회사 PC에서 디스패치를 사용하려면 IT 정책을 먼저 확인하세요. 일부 환경에서는 외부 트리거 자체를 차단하는 경우가 있습니다.
