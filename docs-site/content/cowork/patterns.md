...
시스템이 **합리적 기본값**을 자동 선택합니다. 예를 들어 사업계획서 분량은 15장, 보고서 톤은 격식체로 설정됩니다. 다만 회사 정보·고객 데이터·법률 면책 같은 민감 영역은 사용자가 명시적으로 확인해야 진행합니다.

### Q2. 중간에 멈추고 싶으면?

`/stop` 또는 자연어로 "여기까지 멈춰줘"라고 입력하면 현재 단계의 산출물까지만 저장하고 체인이 중단됩니다.

### Q3. 어떤 스킬이 실제로 실행됐는지 보고 싶으면?

매 작업 후 `*.moai/log/<run_id>/chain.json`에 호출된 스킬, 실행 순서, 소요 시간이 기록됩니다.

### Q4. 매번 같은 질문에 같은 답을 하는 게 귀찮아요.

**프로젝트 메모리**(`.moai/project/profile.md`)에 회사 정보와 기본 설정을 한 번 저장해두면 시스템이 자동으로 참조하여 다음부터는 같은 질문을 반복하지 않습니다. 자세한 방법은 [프로젝트·메모리 가이드](../projects-memory/)를 참고하세요.

### Q5. 4가지 패턴이 섞일 수 있나요?

가능합니다. 예를 들어 **패턴 4 + 패턴 3** 조합으로 매주 월요일 09:00에 `./inbox/` 폴더의 영업 메일을 일괄 분류하는 스케줄+배치 작업을 구성할 수 있습니다. 시스템이 패턴을 자동으로 조합합니다.
...
- **[플러그인 카탈로그](../../plugins/)** — 21 도메인 144 스킬 전체 목록
...
- 핵심 사용 패턴 디자인: 사용자 인터뷰 + AskUserQuestion 자동화 + 스킬 체이닝 통합 (v2.8.0)
