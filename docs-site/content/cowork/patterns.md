...
title: "사용 패턴 가이드"
weight: 28
description: "Cowork는 '긴 프롬프트를 직접 쓰는 도구'가 아닙니다. 짧은 자연어 한 줄 → 시스템이 묻고 → 스킬 체이닝으로 자동 처리. 4가지 표준 패턴을 익히면 177개 스킬을 자유롭게 조합할 수 있습니다."
...
매 작업 후 `*.moai/log/<run_id>/chain.json`에 호출된 스킬·순서·소요 시간이 기록됩니다.

### Q4. 매번 같은 질문에 같은 답을 하는 게 귀찮아요.

**프로젝트 메모리**(`.moai/project/profile.md`)에 회사 정보·기본 설정을 한 번 저장하면 시스템이 자동 참조 — 다음부터 묻지 않습니다. [프로젝트·메모리 가이드](../projects-memory/) 참조.
...
- **[플러그인 카탈로그](../../plugins/)** — 28 도메인 177 스킬 전체 목록
