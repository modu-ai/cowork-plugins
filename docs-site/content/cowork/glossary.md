...
"이런 상황에서는 이렇게 해라"라는 절차적 지침 묶음입니다. 디렉터리 하나(`SKILL.md` + references)가 기본 단위이며, 사용자의 자연어 요청이나 슬래시 명령으로 호출됩니다.

**오개념**: 스킬은 "기능"이 아닙니다. 기능을 수행하는 방법을 담은 지침서입니다.

### 플러그인 (plugin)

여러 스킬·커넥터·서브에이전트를 한 도메인으로 묶어 배포하는 단위입니다. `plugin.json` + `skills/*` + `.mcp.json`으로 구성되며, 설치 한 번으로 그 안의 모든 스킬이 활성화됩니다.

**오개념**: 플러그인과 스킬은 1:1이 아닙니다. 플러그인 하나에 스킬이 1-10개 들어갑니다.

### 스킬 체인 (skill chain)

여러 스킬을 순서대로 엮어 한 요청을 처리하는 파이프라인입니다. 예를 들어 `strategy-planner → docx-generator → ai-slop-reviewer`처럼 도메인 처리, 포맷 변환, 품질 검수 단계가 자동으로 이어집니다.

**오개념**: 체인은 시스템이 자동으로 엮는 것이 아니라, 사용자 요청 문맥에 맞춰 Claude가 판단해 구성합니다.

### 커넥터 (connector)

Slack·Gmail·Notion 등 외부 서비스에 접근하는 인증·API 래퍼입니다. 커넥터를 연결해두면 "Drive에서 이번 주 회의록 찾아줘"처럼 자연어로 외부 서비스를 활용할 수 있습니다. Cowork에서는 MCP로 구현됩니다.

### MCP (Model Context Protocol)

Anthropic이 정의한 커넥터 표준 프로토콜입니다. 커넥터는 그 규격을 따르는 구체적 구현이고, MCP는 규격 자체입니다. 둘은 동의어가 아닙니다.

### 서브에이전트 (subagent)

긴 작업을 나눠 수행하기 위해 메인 세션이 별도 컨텍스트로 위임하는 하위 Claude 인스턴스입니다. 예를 들어 12개 PDF를 동시에 검토할 때 파일별로 서브에이전트를 띄워 병렬 처리합니다.

**오개념**: 서브에이전트는 스킬이 아닙니다. 스킬은 지침이고, 서브에이전트는 그 지침을 실행하는 주체입니다.
