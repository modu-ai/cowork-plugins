...
세 가지 원인을 순서대로 점검합니다. 먼저 플러그인이 설치되어 있는지 `/plugin` 슬래시 명령으로 확인하고, 요청문에 스킬 트리거 키워드가 포함되어 있는지 [스킬 사용법](../skills/)을 참고해 확인하세요. 마지막으로 상위 도메인 스킬에서 포맷 스킬로 명시적 인계가 이루어졌는지 점검합니다.
...
[스킬 체이닝 가이드](../../cookbook/skill-chaining/)의 3원칙(도메인 → 포맷 → 품질)을 먼저 읽은 뒤, 쿡북 예제 중 자기 업무와 가장 가까운 것을 골라 복사·붙여넣기 후 변형하는 것이 가장 빠른 방법입니다.
