...
한국에서 영리 목적 이메일을 발송할 때는 법적 필수 요소 네 가지를 빠뜨리면 과태료 대상이 됩니다. 제목 첫머리에 `[광고]` 표시 또는 본문 상단 광고 표기, 발송자 정보(회사명·연락처·주소), 수신거부(unsubscribe) 한 번 클릭으로 처리, 야간(21시~익일 8시) 발송 시 별도 동의 — 이 네 가지입니다. `email-sequence` 스킬은 이 항목들을 자동으로 본문에 삽입하므로 수동으로 챙길 필요가 없습니다.
...
이메일 마케팅에서 가장 위험한 실수는 법적 표시를 빠뜨리는 것입니다. 정보통신망법 위반은 과태료 대상이므로, 발송 전 반드시 확인하세요. 수신거부를 처리한 이메일 주소에 다시 마케팅을 보내는 것도 자주 발생하는 실수인데, 재구독은 기존 동의와 별개로 새로운 동의가 필요합니다. 이름·구독 일자·관심사 중 하나라도 개인화 요소를 넣으면 오픈율이 5~10%p 오른다는 점도 놓치기 쉽습니다. 마지막으로 HTML 전용으로만 발송하면 일부 이메일 클라이언트에서 깨질 수 있으니, 텍스트 fallback 버전을 함께 준비하세요.
