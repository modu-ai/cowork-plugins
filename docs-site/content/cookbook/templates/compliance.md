...
`moai-legal:compliance-check` 스킬 하나로 규제 준수 점검·내부 감사·ESG 보고·인허가 서류 지원까지 처리합니다. 아래 체크리스트 항목을 프롬프트에 전달하면 스킬이 점검 보고서를 자동 생성합니다.

## 한국 기업이 가장 자주 빠뜨리는 4개 영역

분기 점검에서 빠지기 쉬운 네 개 영역을 체크리스트 형식으로 정리했습니다. 항목별로 담당자가 직접 확인하고 체크하세요.
...
체크리스트를 파일로 갖고 있으면서 실제 점검은 하지 않는 경우가 가장 많습니다. 분기 첫째 주를 점검 주간으로 캘린더에 미리 고정해 두세요. 점검 결과를 PDF 파일로만 보관하는 것도 문제입니다. 미흡 항목은 [트랙 — 법률](../../tracks/track-legal/)의 액션 아이템 생성 패턴으로 즉시 티켓화해야 실제로 처리됩니다. 개인정보처리방침은 한 번 작성하고 방치하기 쉽지만, 처리 항목·위탁사·보유기간이 바뀔 때마다 갱신해야 법적 효력이 유지됩니다.
