...
처음 받은 CSV·Excel의 품질을 빠르게 진단하거나, 결측값·이상값·중복 레코드를 탐지할 때 씁니다. 컬럼별 분포·상관관계를 요약하거나 1만 ~ 수십만 행 규모의 데이터를 탐색할 때도 여기서 시작합니다.
...
500MB 이상 Excel은 탐색만 해도 5분 이상 걸립니다. "처음 1만 행만 샘플링해서"라고 지시하면 속도가 크게 개선됩니다(**큰 파일은 샘플링부터**). SKILL.md 기본값은 결측률 10% 초과 컬럼에만 경고하는데, 더 보수적으로 보고 싶다면 "결측률 3% 초과 경고"를 명시하세요(**결측률 기준**). 범주형 변수는 기본 10개를 보여주지만 "Top 20까지"로 늘릴 수 있습니다(**범주형 변수 Top N**).

## Part 2 — data-visualizer

### 언제 쓰나

Markdown 리포트에 바로 넣을 수 있는 차트가 필요하거나, Chart.js 기반 인터랙티브 대시보드(HTML), Mermaid 흐름도·시퀀스 다이어그램, PPT·Word로 옮길 고정 이미지가 필요할 때 씁니다.
...
공공데이터포털(`data.go.kr`) API나 KOSIS 통계청 데이터를 조회하거나, 내부 데이터를 공공 지표와 비교하거나, KPI 대시보드에 거시 지표를 추가할 때 씁니다.
...
API 키는 절대 코드에 직접 넣지 마세요. 프로젝트 폴더의 `.env` 또는 `credentials.env` 파일에 저장하고 `.gitignore`에 추가하는 것이 기본입니다.
...
SKILL.md 본문에는 환경변수 참조만 넣고, 프롬프트나 로그에 키를 노출하지 않도록 주의하세요.
...
고객 개인정보·금융 거래처럼 민감한 데이터는 별도 암호화 폴더에서만 작업하고, `ai-slop-reviewer` 같은 후처리에 노출되지 않도록 프롬프트에 명시하세요(**민감 데이터는 격리**). 고객 이름·이메일·전화번호는 해시·마스킹 처리 후 분석 대상에 포함하고, "이메일은 도메인만 유지, 로컬파트는 마스킹" 같은 구체적 규칙을 프롬프트에 적어두면 됩니다(**익명화·가명화**). 공공데이터는 대부분 CC-BY 또는 공공누리 라이선스이므로, 보고서에 출처를 명기하는 것을 잊지 마세요(**저작권·출처**).
