...
VC 투자와 별도로 정부지원사업은 지분을 내주지 않는 비희석 자금이기 때문에 매년 빠짐없이 검토할 가치가 있습니다. 마감일을 놓치면 1년을 기다려야 하므로, 임박한 일정을 먼저 파악하는 것이 중요합니다.
...
TAM을 지나치게 넓게 잡는 것이 가장 흔한 실수입니다. "전 세계 ERP 시장"이 아니라 "한국 중견 SaaS ERP 도입 가능 기업"처럼 SAM을 실제 도달 가능한 범위로 좁혀야 투자자가 신뢰합니다. 트랙션 지표도 다운로드·가입자 수 같은 vanity metrics보다 매출·리텐션·NPS를 전면에 내세우세요. 재무 모델은 행이 100개를 넘어가는 순간 설명할 수 없는 모델이 됩니다. 5개 시트로 단순화하고, 가정(assumption)을 물어봤을 때 즉시 답할 수 있는 깊이를 유지하는 것이 중요합니다.
