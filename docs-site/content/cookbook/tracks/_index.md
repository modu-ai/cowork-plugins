...
> 트랙은 **역할·도메인별 표준 워크플로우**입니다. 업무마다 "어떤 스킬을 어떤 순서로 호출해야 하지?"를 매번 고민할 필요가 없습니다. 각 트랙은 [사용 패턴 가이드](../../cowork/patterns/)의 4가지 표준 패턴(단일 프롬프트·멀티턴·배치·스케줄)을 적용한 구체적 시나리오를 담고 있으며, 사용자는 짧은 한 줄만 입력하면 시스템이 나머지를 처리합니다.
...
역할이 다르면 필요한 워크플로우도 달라집니다. 아래 흐름도로 지금 내 상황에 맞는 트랙을 바로 찾아보세요.
...
어떤 트랙을 선택하든 구조가 동일하기 때문에 한 트랙에 익숙해지면 나머지도 빠르게 파악할 수 있습니다. 10개 트랙 + 부록은 아래 동일한 골격을 따릅니다.
