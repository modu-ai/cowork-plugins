...
---
> **이 트랙은 사용자가 직접 자체 플러그인을 빌드하는 고급 튜토리얼입니다.** 일반 사용자가 짧은 자연어 한 줄로 PRD·로드맵·UX 산출물을 만드는 경우는 아래 한 줄 요청 예시를 보세요 — 시스템이 자동으로 처리합니다. 자체 플러그인 빌드(Step 기반)는 본 트랙의 별도 섹션 참조.
>
> **한 줄 요청 예시 (일반 사용)**: "결제 모듈 PRD 초안 + 사용자 인터뷰 가이드 만들어줘" → 시스템 인터뷰 (사용자 페르소나·우선순위·KPI) → 자동 체인 → DOCX + 인터뷰 스크립트 도착
