...
분기 결산 때마다 전표를 정리하고, 재무제표를 만들고, 경영진 보고용 요약까지 준비하다 보면 정작 분석에 쓸 시간이 남지 않습니다. 재무 트랙은 결산·세무·분석·보고서 생성을 하나의 자동 흐름으로 연결합니다. 처음 사용 시 회사 정보를 한 번 입력해두면 이후 시나리오에서 같은 인터뷰를 반복하지 않아도 됩니다.
...
| 4 | "투자자용 재무 분석 보고서 만들어줘" | financial-statements → financial-ratio → docx → ai-slop |
...
`financial-statements`(3개년 통합) → `financial-ratio`(ROA·ROE·부채비율·유동비율 등) → `variance-analysis`(YoY 변동률) → `docx-generator` 또는 `pptx-designer` → `ai-slop-reviewer`
