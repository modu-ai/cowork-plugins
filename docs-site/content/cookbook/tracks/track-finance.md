...
---
> **이 트랙은 사용자가 직접 자체 플러그인을 빌드하는 고급 튜토리얼입니다.** 일반 사용자가 짧은 자연어 한 줄로 재무 산출물을 만드는 경우는 아래 한 줄 요청 예시를 보세요 — 시스템이 자동으로 처리합니다. 자체 플러그인 빌드(Step 기반)는 본 트랙의 별도 섹션 참조.
>
> **한 줄 요청 예시 (일반 사용)**: "Q1 변동분석 + K-IFRS 보고서 만들어줘" → 시스템 인터뷰 (회사 단계·산업·기간) → 자동 체인 → XLSX + DOCX 도착
...
- [financial-statements 재무제표 생성 가이드](../../../plugins/moai-finance/)
- [tax-helper 세무 처리 전문](../../../plugins/moai-finance/)
- [close-management 결산 관리](../../../plugins/moai-finance/)
- [variance-analysis 예산 분석](../../../plugins/moai-finance/)
- [xlsx-creator 재무 리포트 생성](../../../plugins/moai-office/)
