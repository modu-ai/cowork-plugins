...
데이터 분석 업무는 데이터를 구하는 일부터 시작됩니다. 공공 API를 뒤지고, 수집된 데이터를 정제하고, 의미 있는 시각화를 만들고, 그 결과를 보고서로 다듬어야 비로소 전달할 수 있는 인사이트가 됩니다. 데이터 분석 트랙은 이 전 과정을 공공데이터 수집 → EDA → 시각화 → 보고서 생성 순으로 자동 체이닝합니다. 데이터 분석가·리서치·기획자가 자연어 한 줄로 요청하면, 시스템이 맥락을 수집하고 적절한 스킬을 선택해 실행합니다.
...
이 트랙은 공공데이터 분석 리포트, 시장 조사 및 트렌드 분석, 경영 분석 및 KPI 모니터링, 정책 영향도 분석 등 데이터를 다루는 다양한 업무에 적용됩니다. 사용 플러그인은 공공데이터 수집·분석을 담당하는 **moai-data**, 시각화·보고서 생성을 담당하는 **moai-office**, AI 품질 검수를 담당하는 **moai-core** 세 가지입니다.
...
세 가지 예시 외에도 더 넓은 자동화가 가능합니다. 예를 들어 매일 09:00에 자동으로 데이터를 수집해 대시보드를 갱신하거나([패턴 4: 스케줄 자동화](../../../cowork/patterns/) 적용), 시계열 데이터로 다음 분기 예측값과 신뢰 구간을 산출하거나, 주간·월간 리포트를 Slack·이메일로 정기 발행하는 방식으로 확장할 수 있습니다.
...
공공데이터를 분석할 때는 출처와 수집 방법을 문서에 명시해야 합니다. 통계적 유의성을 검증하고 인과관계와 상관관계를 구분해 오해를 방지하며, 데이터의 시간적 한계와 개인정보보호 규정도 함께 고려해야 합니다.
