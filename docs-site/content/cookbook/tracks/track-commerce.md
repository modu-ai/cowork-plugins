...
D2C 셀러가 신상품을 출시할 때 상세페이지·광고 소재·프로모션 메시지·재구매 시퀀스를 제각각 준비하면 몇 주가 걸립니다. 이커머스 트랙은 시장조사에서 상품 기획, 콘텐츠 자동화, CRM 운영, 수익 분석까지 5단계를 하나의 흐름으로 연결합니다. 한 줄 요청 하나로 시스템이 필요한 스킬을 자동 선택해 체이닝하며, 브랜드 정보는 한 번 저장해두면 이후 질문이 줄어듭니다.
...
**상황**: 무선 이어폰을 스마트스토어와 자사몰에 동시 출시해야 하는데, 13섹션 상세페이지 기획부터 카피·이미지 생성·AI 검수까지 직접 처리하면 며칠이 걸립니다. 상품 정보와 채널을 입력하면 시스템이 전 과정을 자동으로 처리합니다.
...
이커머스 시나리오에서 시스템이 자주 묻는 항목입니다. 한 번 답하면 `.moai/project/profile.md`에 저장되어 다음 시나리오부터 자동 참조하므로 반복 인터뷰가 크게 줄어듭니다.
