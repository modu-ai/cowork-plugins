...
채용공고를 쓰고, 이력서를 분류하고, 면접 질문을 준비하는 일은 HR 팀에서 가장 많은 반복이 일어나는 영역입니다. 구직자 입장에서도 이력서와 포트폴리오를 포지션에 맞게 다듬고, 면접을 준비하는 데 상당한 시간이 걸립니다. HR·커리어 트랙은 채용 측(moai-hr)과 구직 측(moai-career)이 각각 필요한 워크플로우를 한 줄 요청으로 자동 처리합니다.
