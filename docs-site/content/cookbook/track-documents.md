...
주간·월간 보고서, 제안서·사업계획서, 계약서·약관 초안, 정부 지원사업 신청서처럼 줄글과 표가 섞인 문서에 가장 잘 맞습니다.
...
Word 표를 스킬이 자동 생성하지만 열이 너무 많으면 페이지를 벗어납니다. 6열 이내로 유지하거나 가로 방향 페이지를 지시하세요(**표 서식이 깨짐**). 서버에 맑은고딕이 없으면 한글 글꼴이 깨질 수 있는데, SKILL.md에 "기본 글꼴 = 나눔고딕" 지시를 추가하면 됩니다(**한글 글꼴이 깨짐**). 목차는 자동으로 생성되지 않으므로 "목차 포함"이라고 명시해야 Heading 1·2 기준 TOC가 삽입됩니다(**목차 미생성**).

## PowerPoint (PPTX)

### 적합한 업무

주간 KPI 대시보드, IR 피치덱, 임원 보고·경영 전략, 교육·세미나 자료처럼 발표를 전제로 한 시각 중심 문서에 적합합니다.
...
PPT 스킬은 이미지를 별도로 지정해야 삽입합니다. `moai-media:nano-banana`로 먼저 이미지를 만들고 경로를 전달하세요(**이미지가 들어가지 않음**). 슬라이드 수 상한을 지정하지 않으면 15장 이상으로 부풀어 오를 수 있으니 "5-7장"처럼 명시하세요(**슬라이드 수가 폭주**). 한 슬라이드에 Bullet을 5개 이하로 제한하지 않으면 가독성이 무너지므로, 프롬프트에 상한을 적어두는 것이 좋습니다(**글머리표 중복**).

## Excel (XLSX)

### 적합한 업무

재무 모델·예산 시트, KPI 대시보드, 데이터 정제·피벗, 간트차트처럼 수식과 숫자가 중심인 문서에 적합합니다.
...
"이 수식은 하드코딩되지 않고 셀 참조로"라고 명시하지 않으면 값만 채워지는 경우가 있습니다(**수식이 하드코딩됨**). 차트가 별도 시트에 생성되는 경우엔 "임베드된 차트로 요약 시트에 포함"이라고 지시하세요(**차트가 별도 시트에 생성됨**). CSV 파싱 시 날짜가 텍스트로 저장되는 경우가 자주 있으니 "date 컬럼은 날짜 형식으로"라고 명시하세요(**날짜 형식이 텍스트로 저장됨**).

## 한글 (HWPX)

### 적합한 업무

정부·공공기관 제출 문서, 기안서·사내 공문, 보조금 신청서, 입찰 서류처럼 한컴오피스 양식이 요구되는 경우에 씁니다.
...
HWPX는 한컴오피스가 없어도 열람할 수 있지만 일부 양식은 HWP 전용입니다. 관공서에 HWP·HWPX 둘 다 허용되는지 먼저 확인하세요(**한글 프로그램 설치 여부**). 한글 특유의 표 서식을 완벽하게 재현하기는 어려우므로, 양식 제출 전 한컴오피스에서 열어 한 번 점검하는 것이 안전합니다(**표 정렬이 미묘하게 어긋남**).

## PDF

### 적합한 업무

최종 배포용 리포트(편집 불가), 폼 양식 입력(입사 지원서·계약서), 여러 PDF 병합·페이지 추출, 스캔 PDF 텍스트 추출처럼 파일을 가공하거나 내용을 읽어내야 할 때 씁니다.
...
저해상도 스캔 PDF는 OCR 오인식이 많으므로 300dpi 이상의 스캔 파일을 사용하세요(**스캔 PDF OCR 정확도**). 일부 PDF는 필드가 이미지로 렌더링되어 있어 자동 입력이 불가능한데, 이런 경우에는 [PDF Tools MCP](https://github.com/modelcontextprotocol/servers)의 `bulk_fill_from_csv`로 우회할 수 있습니다(**폼 필드가 자동 채워지지 않음**).
