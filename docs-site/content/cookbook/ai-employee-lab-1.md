...
>
> **주의**: 본 랩은 **사용자가 직접 플러그인을 빌드하는 고급 실습**입니다. 일반 사용자가 재무 산출물(브리핑·이상 감지·분개)을 만들 때는 "매일 아침 모닝 브리핑 슬랙으로 보내줘" 같은 한 줄 요청 → 시스템 자동 처리만으로 충분합니다 ([사용 패턴 가이드 참조](../../cowork/patterns/)). Step 분할 학습은 자체 플러그인 빌더를 위한 것.
