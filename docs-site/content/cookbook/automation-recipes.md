...
자리를 비운 동안에도 AI가 계속 돌아가길 바란다면 두 기능을 조합하면 됩니다. 아래는 현장에서 검증된 레시피를 본부별로 정리한 목록입니다.
...
Schedule은 `/schedule` 슬래시 명령이나 "매일 아침 8시에 X 실행해줘" 같은 자연어 한 줄로 등록합니다. 시간 기준은 로컬 PC 시계(UTC가 아님)이므로 서머타임 구간에 주의해야 합니다. PC가 켜져 있고 Cowork가 실행 중일 때에만 발동하며, 꺼진 PC에서는 해당 회차가 그냥 지나갑니다. 외부 MCP에 쓰기 동작이 포함된 경우에는 최초 1회 명시적 승인이 필요합니다.

## Dispatch — 모바일에서 PC로 지시 보내기
...
1. **작업받을 PC를 마지막으로 사용한 상태로 두고 나가기** — Dispatch는 "마지막으로 사용한 PC"로 라우팅합니다. 사무실 PC가 야간 지시를 받으려면 퇴근 전에 집 PC를 켜지 않는 것이 중요합니다.
2. **집 PC를 잠깐 열어도 라우팅이 바뀝니다** — 저녁에 집 PC로 메일 하나만 확인해도 이후 지시는 집 PC로 향합니다. 중요한 야간 배치가 예약되어 있다면 집 PC를 아예 켜지 마세요.
3. **출장 전 사무실 PC 대기 상태 확인** — 절전 모드, 자동 종료 스케줄, Windows Update 재부팅을 미리 모두 해제해두어야 합니다.
...
모바일 Claude 앱에서 Dispatch를 사용할 때는 다음 세 가지 패턴이 자주 쓰입니다.
