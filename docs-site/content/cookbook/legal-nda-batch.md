...
인박스에 NDA가 쌓이는 속도가 검토 속도를 이길 때가 있습니다. 이 레시피는 스타트업 법무팀(인하우스 1-3명), 법무법인 어소시에이트, 사업개발 PM이 NDA를 자체적으로 1차 검토할 때를 위한 것입니다.
...
5단계가 순서대로 이어집니다. `nda-triage`가 상호/일방·기간·손해배상 캡·준거법 등을 분류하고, `contract-review`가 조항별로 정밀 검토해 수정 제안 마크업을 붙입니다. `legal-risk`가 종합 위험도(상/중/하)와 우선순위를 평가하면, `docx-generator`가 위험 보고서 DOCX를 생성하고, `ai-slop-reviewer`가 최종 어투를 정리합니다.
...
NDA가 아닌 일반 계약서를 일괄 처리하려면 `nda-triage` 대신 `contract-review`로 시작하고, 계약 유형별로 폴더를 나눠 분리 실행하면 됩니다(**계약서 일괄 검토**). 개인정보보호법 준수 여부까지 확인해야 한다면 검토 후 `compliance-check`를 체인에 추가하세요(**팀 합의 워크플로우**). 매달 이 체인을 반복한다면 `/nda-batch` 슬래시 명령으로 저장해두면 한 번에 전체 체인을 실행할 수 있습니다(**반복 사용**).
