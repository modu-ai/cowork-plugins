...
사업계획서 본문을 AI가 만들었어도, 결국 팀장에게 보내는 건 Word 파일이고 투자자에게 내미는 건 PPT입니다. `moai-office`는 그 마지막 저장 단계를 담당합니다. 한국 기업·관공서에서 실제로 쓰이는 문서 양식으로 PowerPoint(PPTX), Word(DOCX), Excel(XLSX), 한글(HWPX), 다국어 PDF 다섯 가지 포맷을 자동 생성하며, 폰트·결재란·숫자 표기·표 라벨 등 한국 문서 관례를 기본값으로 반영합니다.

각 스킬은 검증된 오픈소스 라이브러리(pptxgenjs, python-docx, openpyxl, python-hwpx, PyMuPDF) 위에서 동작합니다. 다른 도메인 플러그인이 만든 산출물(예: 사업계획서 본문, 결산 데이터)을 받아 최종 파일로 저장하는 **출력 단계**를 전담하며, `pdf-writer`는 Noto Sans CJK 번들로 한·중·일 글리프 누락 없이 PDF를 생성합니다.

## 설치 시 유의

Python 3.10 이상을 권장하며, `python-hwpx`는 pip로 별도 설치해야 합니다(`pip install python-hwpx --break-system-packages`). HWPX 파일의 폰트가 올바르게 나오려면 시스템에 Pretendard·맑은고딕·바탕이 설치되어 있어야 합니다.
...
공문서의 결재란·두문·본문·결문 구조가 기본값으로 적용됩니다. HWPX는 아래한글 2020 이상과 호환되며, 표·차트 라벨은 한국어로 출력되고 숫자 구분은 한국식 콤마(예: 12,345,678)를 따릅니다.
