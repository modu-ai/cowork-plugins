---
title: "moai-core — 프로젝트·라우터·AI 슬롭 + MCP 3커넥터 셋업"
weight: 20
description: "moai-core는 cowork-plugins의 기반 플러그인으로 프로젝트 초기화, AI 슬롭 검수, 피드백 허브, MCP 3커넥터 인증 가이드를 포함한 8개 핵심 스킬을 제공합니다."
...
이 인프라 층이 구체적으로 까는 것은 크게 세 가지입니다. 첫째, 프로젝트의 도면 역할을 하는 작업 지침서(`CLAUDE.md`)를 자동으로 그려주는 `/project` 마법사, 둘째, 완성된 글에서 기계적으로 튀어나오는 AI 특유 어투를 마지막 한 번 솎아내는 `ai-slop-reviewer`, 셋째, 외부 도구(Drive·Notion·Higgsfield)를 연결하는 발주 창구 겸 가이드인 `mcp-connector-setup`입니다. 이 세 가지가 자리잡아야 비로소 다른 플러그인이 만든 산출물이 안정적으로 흘러나옵니다.

`moai-core`는 `cowork-plugins` 마켓플레이스의 모든 도메인 플러그인이 공유하는 인프라를 제공합니다. 프로젝트별 작업 지침(`CLAUDE.md`)을 자동 생성하고 프로젝트 맞춤 에이전트까지 합성하는 `/project` 마법사, 모든 텍스트 산출물의 마지막 단계에서 AI 패턴을 다듬어주는 `ai-slop-reviewer`, 버그·기능 요청을 GitHub Issues로 바로 등록하는 `feedback` 스킬, Drive·Notion·Higgsfield **3커넥터 인증·환경변수·트러블슈팅 통합 가이드** `mcp-connector-setup`을 포함한 **총 8개 스킬**이 포함되어 있습니다.
...
`/project` 한 번이면 설치된 `moai-*` 플러그인을 자동 감지해 산출물별 스킬 체인을 설계하고, **코디네이터 에이전트까지 동적 스캔**(`agents/*.md` frontmatter 인벤토리)해 멀티스텝 체인을 에이전트 우선으로 매핑합니다. 200라인 이내의 `CLAUDE.md`를 프로젝트 루트에 생성하고, 자격 조건을 갖춘 워크플로우에 한해 프로젝트 맞춤 sub-agent를 `.claude/agents/`에 합성합니다. (`/project init`은 레거시 별칭으로 계속 동작합니다.)
...
| `mcp-connector-setup` | Drive·Notion·Higgsfield **3커넥터** 인증·환경변수·트러블슈팅. Windows MAX_PATH·한글 파일명 30자·`computer://` 링크 오류 대응. 셋업 완료 체크리스트(3커넥터 인증 + 1회 호출 성공) | "MCP 커넥터 연결", "Drive 인증 방법", "Higgsfield 키 발급", "Windows MAX_PATH 오류" |
...
문제는 **기존 조수가 아무도 담당하지 않는 일**입니다. 이때는 그 빈자리를 채우는 전담 에이전트를 새로 짜서 `.claude/agents/` 폴더에 기록합니다. 단, 무조건 만드는 게 아니라 (1) 반드시 거쳐야 할 정해진 단계가 있거나, (2) 여러 결과물을 동시에 찍어내야 하거나, (3) 사용자가 매일처럼 반복하는 일일 때만 만듭니다. 그냥 한두 단계짜리 단순 작업은 굳이 에이전트를 새로 만들지 않고, 기존 스킬 몇 개를 차례로 부르는 것으로 충분하다고 판단합니다.
...
```mermaid
flowchart TD
    subgraph INPUT["입력 — 설치된 cowork 플러그인 (사용자 환경)"]
        direction LR
        IN_SKILL["스킬<br/>(단일 작업 도구)<br/>예: pptx-designer"]
        IN_COORD["코디네이터 에이전트 (번들)<br/>여러 스킬을 묶은 완성형 실행기<br/>예: business-plan-coordinator"]
        IN_CTX["프로젝트 맥락<br/>(인터뷰로 수집)<br/>어떤 산출물을 얼마나 자주 만드나"]
    end

    SCAN["① SCAN — 인벤토리 (Phase 2)<br/>설치된 플러그인의 agents/*.md frontmatter 동적 스캔<br/>코디네이터+스킬 전체 목록을 inventory.json으로 도출 (하드코딩 X)"]
    MATCH{"② 매칭되는 코디네이터가 있는가?<br/>(Phase 3 결정 규칙)"}
    REUSE["③-A 기존 코디네이터 우선 (재사용 경로)<br/>CLAUDE.md 체인 표에 해당 코디네이터를 실행기로 지정<br/>새 에이전트 생성 없음"]
    QUALIFY{"③-B 자격 판정 (Phase 3.5)<br/>아래 중 하나 이상 만족?<br/>(a) 고정 다단계 + 비우회 게이트<br/>(b) 병렬 fan-out (여러 산출물 동시)<br/>(c) 사용자가 매우 자주 반복"}
    SYNTH["④ SYNTHESIZE — 신규 전담 에이전트 합성<br/>.claude/agents/&lt;name&gt;.md 작성<br/>도구: Read, Grep, Glob, Write, Edit, WebSearch"]
    INLINE["⑤ 인라인 스킬 체인 (충분함 경로)<br/>단순 1-2단계이거나 자격 미충족 → 에이전트 생성 X<br/>CLAUDE.md에 스킬 나열로 처리"]

    subgraph OUTPUT["산출물 — 프로젝트에 기록됨"]
        direction LR
        OUT_CLAUDE["CLAUDE.md (200라인 이내)<br/>산출물별 체인 표 — 실행기가 코디네이터면 [에이전트], 없으면 스킬 나열"]
        OUT_AGENTS[".claude/agents/&lt;name&gt;.md<br/>프로젝트 맞춤 서브에이전트<br/>빈 자리만 합성됨, 번들보다 우선순위 높음"]
    end
    ACTIVE["다음 세션에서 활성화<br/>디스크의 에이전트는 세션 시작 시 로드<br/>새 세션(또는 /reload-plugins)부터 자연어 위임으로 자동 호출"]
    ...
    classDef reuse fill:#788C5D,stroke:#5F6F4A,color:#FFFFFF,font-weight:bold
    classDef plain fill:#FFFFFF,stroke:#D1CFC5,color:#141413
    classDef decision fill:#FAF9F5,stroke:#D97757,color:#141413

    class SYNTH highlight
    class REUSE reuse
    class IN_SKILL,IN_CTX,SCAN,INLINE,OUT_CLAUDE plain
    class MATCH,QUALIFY decision
```
...
```text
MCP 커넥터 3개 연결 방법 알려줘 — Drive·Notion·Higgsfield
```
→ `mcp-connector-setup`

## `mcp-connector-setup`

Drive·Notion·Higgsfield 3커넥터를 Cowork에 연결하는 단계별 가이드입니다. 커넥터별 인증 절차·환경변수 설정·트러블슈팅을 한 곳에서 다루며, 셋업 완료 체크리스트는 **3커넥터 모두 인증 성공 + 1회 호출 성공**입니다.
...
여기서 두 낯선 용어가 등장합니다. **MCP**(Model Context Protocol)는 그 전화선 규격, 즉 "AI가 외부 도구와 통신하는 표준 연결 방식"입니다. **커넥터**는 그 규격에 맞춰 깔은 전화선 한 가닥 — Drive 선, Notion 선, Higgsfield 선 3가닥입니다. 그리고 **API 키**(또는 OAuth 토큰)는 "이 전화를 걸 권리가 있는, 인증된 전화번호" 역할을 합니다.

`mcp-connector-setup`은 바로 이 **전화선 3가닥을 설치하고 인증된 전화번호를 등록해 주는 통신 설치 가이드**입니다. 각 커넥터가 어떤 인증을 요구하는지, 환경변수(설정값)를 어디에 적어 넣어야 하는지, 선이 안 깔리거나 번호가 만료됐을 때(트러블슈팅) 어떻게 고치는지를 차례로 안내합니다. 3가닥이 모두 연결되고 한 번씩 "여보세요" 호출이 성공해야 셋업이 끝난 것으로 봅니다.
