...
네이버 블로그에 쓴 글이 인스타그램 카드뉴스에 그대로 맞지 않듯, 채널마다 알고리즘과 포맷 기준이 다릅니다. `moai-content`는 그 채널별 차이를 반영해 설계된 텍스트 콘텐츠 생성 플러그인입니다. 네이버 C-Rank·D.I.A. 알고리즘, 인스타그램 2026년 캐러셀 길이 기준 등 실제 운영 노하우가 본문 구조에 내장되어 있습니다.

블로그 포스트·카드뉴스·랜딩페이지·뉴스레터·**상세페이지 기획**·상세페이지·SNS·카피라이팅·미디어 기획·한국어 맞춤법·**한국어 AI 티 정밀 윤문**·**마크다운 보고서 HTML 변환**까지 12개 스킬이 도메인별로 분리되어 있어 필요한 채널만 골라 호출할 수 있습니다. 별도 API 키 없이 사용 가능하며, WordPress 자동 업로드를 원할 때만 WordPress MCP 연결이 필요합니다.
...
네이버 **C-Rank·D.I.A. 알고리즘**을 반영한 본문 구조가 기본으로 적용되고, **GEO(생성형 검색 최적화)** 시대에 맞춰 FAQ·스키마 마크업을 권장합니다. 인스타그램 **2026년 알고리즘** 변화에 맞춘 캐러셀 길이 기준과 네이버 블로그·티스토리의 **SEO 점수** 체크포인트도 내장되어 있습니다.
