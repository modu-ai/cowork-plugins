...
세금 신고 시즌이 되면 프리랜서는 3.3% 원천징수 영수증을 한 장씩 집계하고, 경리 담당자는 홈택스 메뉴를 뒤지며 부가가치세 신고 마감을 쫓습니다. 월말 결산 때는 K-IFRS 기준을 맞추느라 재무상태표·손익계산서·현금흐름표를 반복 수정하고, 분기마다 예산 대비 실적을 뽑아 경영진 보고서를 만들어야 하죠. 기준 자체가 해마다 바뀌는 탓에 요율 하나를 확인하러 국세청 고시를 다시 찾아보는 일도 잦습니다.

`moai-finance`는 이 한국 기준 재무·세무 실무 전반을 자동화합니다. 원천징수 계산부터 홈택스 신고 절차 안내, K-IFRS 재무제표 작성, 4대보험 정산, 예실 분석까지 6개 스킬이 한 플러그인에 묶여 있습니다. 2026년 K-IFRS 제1118호 변경사항과 4대보험 요율·노동법 최신값이 이미 반영되어 있어 별도로 기준을 찾아볼 필요 없이 바로 사용할 수 있습니다.
...
2026년 K-IFRS 제1118호 변경사항이 반영되어 있고 4대보험 요율과 노동법 최신값이 내장되어 있습니다. 홈택스 신고 일정은 스킬이 자동으로 안내하므로 마감을 놓칠 걱정이 줄어듭니다.
...
대법원이 운영하는 공식 **법원경매정보**(`courtauction.go.kr`)의 매각공고와 사건정보를 read-only로 조회합니다. 자산 처분·경매 투자·실사 검토 목적으로 활용합니다.

### Throttling — 매우 중요

사이트는 자동화 호출에 매우 민감해 **빠른 연속 조회 시 IP가 약 1시간 차단**될 수 있습니다. 스킬은 호출 간 최소 **2초** 지연과 세션당 **10회** budget을 강제합니다. 차단이 발생하면 자동 retry 없이 즉시 멈추고 사용자에게 알립니다. 자동 retry는 차단 시간을 연장할 수 있어 허용하지 않습니다.

### Honest framing (사용자에게 항상 알려야 할 사실)

1. 데이터는 **공고 시점 기준**이며 정정·취하·연기로 변경될 수 있습니다.
2. **실제 입찰 전에는 법원 원문을 재확인**해야 합니다.
3. 본 스킬은 **read-only**입니다. 입찰서 자동 작성·자동 제출은 절대 미지원합니다.
...
KRX(한국거래소) 상장 종목 검색·기본정보·일별 시세를 조회합니다. `moai-business`의 DART(공시)와 함께 쓰면 공시 내용과 시세 흐름을 한 번에 파악할 수 있습니다.
...
`KRX_API_KEY` 발급은 사용자가 직접 할 필요 없습니다. NomaDamas hosted 프록시가 키를 보유하므로 바로 호출할 수 있습니다. self-host 환경이 필요하면 `KSKILL_PROXY_BASE_URL`로 대체합니다.

### 데이터 갱신과 보유 기간

데이터는 영업일 기준 하루 뒤 13시 이후에 갱신됩니다(예: 금요일 데이터는 차주 월요일 13시 이후). 2010년 이후 일별 snapshot을 제공하며, 실시간 호가·체결은 미제공입니다.
