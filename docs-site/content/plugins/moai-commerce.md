---
title: "moai-commerce — 한국 이커머스 풀스택 30 스킬"
weight: 25
description: "한국 D2C·오픈마켓·자사몰 셀러를 위한 30개 스킬 묶음 — 시장조사·고객 분석·상품명·상세페이지·채널 메시지·통합 전략·광고 최적화·마진/LTV 설계·자동화 진단·법규 준수·프로모션·재구매·이미지 파이프라인·리뷰 통합·VOC·구독·인플루언서·식약처 안전."
...
> 한국 이커머스 운영의 전 사이클을 한 플러그인에 담은 **30개 스킬** 묶음입니다. 시장조사부터 고객 분석·상품명·상세페이지·채널 메시지·통합 전략·광고 최적화·마진/LTV 설계·자동화 진단·법규 준수·프로모션·재구매·이미지 파이프라인·리뷰 통합·VOC·구독·인플루언서·식약처 안전까지 자연어 한 줄 입력으로 자동 체인 호출됩니다.
...
`moai-commerce`를 한마디로 비유하면 **주방장 30명이 있는 대형 키친**입니다. 주방장마다 담당이 다릅니다. 시장조사 담당은 재료 시세를 조사하고, 카피라이터 담당은 상세페이지 문구를 쓰고, 디자이너 담당은 이미지를 합성하고, 회계 담당은 마진과 광고 수익을 계산하고, 법규 담당은 광고성 메시지 과태료를 피해줍니다. 손님인 사용자가 "무선 이어폰 상세페이지 만들어줘"라고 주문하면, 주방장 30명이 알아서 분업해 한 그릇(완성된 상세페이지 PNG, 또는 아침 브리핑 한 줄)을 내놓습니다. 스킬 30개가 각자의 일만 담당한 뒤 결과를 조립해 하나의 산출물로 완성하는 구조, 이것이 바로 이 플러그인의 본질입니다.
...
- **운영 데이터 통합**: `commerce-morning-brief`(어제 주문·신규 문의·트렌드·ROAS 4영역 1줄 통합)
...
| `marketplace-coupang-ads` | 쿠팡 광고 입점·운영 가이드 — 광고 유형별 정책·입찰·소재 | 쿠팡 광고 운영 가이드 |
...
## HITL Golden Rule — 80% 자동화, 그러나 10배 더 꼼꼼한 검수

`commerce-automation-audit`와 `commerce-integrated-strategy`가 공통으로 따르는 핵심 원칙입니다. HITL(Human-in-the-Loop)은 "사람이 루프 안에 있는 자동화"라는 뜻으로, 기계가 다 처리하고 끝내는 게 아니라 반드시 사람이 중간에 끼어든다는 의미입니다.

구체적인 규칙은 두 줄입니다. 첫째, 반복되는 업무의 **80%는 AI가 자동으로 초안을 만듭니다**. 예를 들어 하루 100건 들어오는 주문 확인·고객 문의 답변·광고 카피 초안 같은 작업은 기계가 빠르게 처리합니다. 그중 고난도 창의 작업(브랜드 스토리, 핵심 협상, 전략 판단) 20%는 사람이 직접 손으로 합니다.

둘째, 여기가 핵심인데 — 자동으로 만든 결과물은 사람이 직접 한 20%보다 **10배 더 꼼꼼하게 검수합니다**. 왜 그럴까요? AI가 빠르게 뽑은 답변은 양불량이 섞여 있기 때문입니다. 사람이 직접 쓴 글은 자기가 쓴 만큼 어디가 약한지 알지만, 기계가 만든 80건은 "다 됐겠지" 하고 넘기면 오류가 고스란히 고객에게 갑니다. 그래서 자동화 비율이 높을수록 검수 강도를 역으로 높이는 것입니다. 실제로는 "AI가 10초 만에 만든 상세페이지 초안을 사람이 10배의 시간과 꼼꼼함으로 사실 확인·법규·톤앤매너를 점검한다"는 흐름입니다. 즉 자동화는 '시간 절약'이 목표가 아니라 '검수에 쏟을 시간을 만드는' 도구입니다.

```mermaid
flowchart TD
    IN["반복 업무 100건<br/>(주문·문의·카피 초안 등)"]

    subgraph AUTO["자동화 영역 — AI가 담당 (80%)"]
        AI["AI 초안 자동 생성<br/>(80건 = 80%)"]
        GATE{"사람 검수 게이트<br/>(HITL)"}
        DEEP["★ 10배 검수<br/>자동 산출물을 안 한 20% 대비<br/>10배 꼼꼼히 확인"]
    end

    subgraph MANUAL["수동 처리 영역 — 사람 직접 (20%)"]
        DIRECT["고난도·창의 작업<br/>사람이 직접 (20건 = 20%)"]
        REVIEW["표준 검수<br/>(1배 기준선)"]
    end

    OUT["최종 산출<br/>승인 / 반려 후 반영"]
