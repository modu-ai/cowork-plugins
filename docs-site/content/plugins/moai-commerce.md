...
weight: 25
description: "한국 이커머스 셀러의 시장조사·고객 분석·상품명·상세페이지·채널 메시지·통합 전략·광고 최적화·자동화 진단·마진 계산·LTV/CAC 설계·앱 푸시·법규 준수·프로모션·재구매·이미지 파이프라인·리뷰 통합·VOC·구독·인플루언서·얼리팬·트렌드·시즌 캘린더·로컬 재고 조회 36 스킬로 한국 D2C 풀스택 자동화."
...
> 한국 이커머스 운영의 모든 단계를 한 플러그인 안에 담은 풀세트 36 스킬. 시장조사·고객 분석·상품명·NCM 메시지·통합 전략·광고 최적화·자동화 진단·마진 계산·LTV/CAC 설계·앱 푸시·법규 준수·프로모션 기획·재구매 골든타임·상품 이미지 파이프라인·리뷰 통합·VOC 우선순위·구독 모델·인플루언서 협업·얼리팬 부트스트랩·트렌드 변환·시즌 캘린더·로컬 재고 조회까지 자연어 한 줄 입력으로 자동 체인 호출.

## 무엇을 하는 플러그인인가

`moai-commerce`는 한국 이커머스 셀러가 실제로 마주치는 작업 전부를 다룹니다.
...
- **V6 6도구**: 모두의 커머스 3일 마스터 캠프 Day 2 V6 7교시 구조 1:1 매핑 — 시장조사 4 MCP, JTBD 9 + 페르소나 3, 상품명 3안+검증, NCM 메시지 15종, 통합 전략 1장
- **Day 1 셋업**: 매장 운영 데이터 1줄 통합 — `commerce-morning-brief`(어제 주문·신규 문의·트렌드·ROAS 4영역) + `commerce-order-summary`(스마트스토어+카페24+아임웹 채널 통합)
...
- **로컬 재고 조회**: 다이소, 편의점, 마트, 올리브영 상품·매장·재고 조회
...
| 스킬 | 역할 | 대표 출력 |
|---|---|---|
| `commerce-review-aggregator` | 멀티채널 리뷰 통합 분석 — 네이버 스마트스토어·쿠팡·자사몰·YouTube·인스타그램 5채널 → 감정·키워드·인사이트·액션플랜 4단 분석 | 채널별 수집 가이드(API·CSV·Graph API) + 미리캔버스 PPT 자동화 |
| `commerce-voc-triage` | VOC 우선순위 판별 — 3축 분류(고객 핏 ×1~3 / 빈도 ×1~3 / 핵심 가치 ×1~3) + KTAS 응급실 5단계 매핑 | 등급별 응답 톤·내용 가이드 (Level 1 즉시 <1시간 / Level 5 비응급 <1주) |
| `commerce-subscription-strategist` | 구독 비즈니스 모델 설계 — 5가지 질문 자기진단 + 4 구독 모델 분류(소비재/경험/관계/맞춤) + 한국 시장 적합성 진단 | 락인 vs 이탈 방지 메시지 매트릭스 6단계 |
| `commerce-influencer-collab` | 인플루언서·UGC 협업 가이드 — 5 티어(메가/매크로/마이크로/나노/메가나노) × 협업 비용·전환율 매트릭스 + 뒷광고 회피 6항목(표시광고법 + 공정위) | UGC 리그램 가이드 + 5축 굿즈 기획 (과태료 5,000만원 회피) |
| `commerce-early-fan-builder` | 신생 브랜드 충성 100명 부트스트랩 — 5원칙(광고 0원·1:1 손편지·UGC 100%·비공개 채널·추천) + 30일 로드맵 + 100명→1만명 전환 시나리오 | 핵심 지표(재구매율 80%+·평균 10명 추천·LTV/CAC ratio 3+) |
| `commerce-trend-namer` | 네이버 데이터랩 트렌드 변환 — 카테고리별 급상승 검색어·시즌 트렌드 수집 | 상품명 변형 3안 + 해시태그 세트 30개(카·트·브·일) + 블로그 제목 SEO 3안 |
| `commerce-season-calendar` | 연간 시즌 캘린더 — 한국·글로벌 30+ 시즌 이벤트(설날·발렌타인·졸업·가정의 달·휴가·추석·빼빼로·블프·솽스이·크리스마스) + 카테고리별 매출 피크 매핑 | 분기 캠페인 사전 준비 계획(D-30/D-60/D-90/D-120) |

### 프로모션·재구매·이미지 파이프라인 (3)

| 스킬 | 역할 | 대표 출력 |
|---|---|---|
| `commerce-promotion-planner` | 3대 프로모션 기획법(이슈화·얼리버드·한정) — 브랜드 단계 × 목표 매트릭스 + 명목·스토리·혜택 3종 세트 + 벤치마킹 케이스 3개 + 노션 템플릿 8섹션 자동 생성 | 비플레인 '듣보잡' 스몰 D2C 12배 매출 케이스 실전 매뉴얼 |
| `commerce-repurchase-timer` | 재구매 골든타임 3구간 모델(리마인드 0.8T / 데드라인 1.1T / 휴면 1.5T) + 구간별 메시지 톤·채널(앱 푸시·카톡 친구톡·이메일) + 인센티브 강도(0~5%/10~15%/25~40%+사은품) + 리드 스코어링 8개 행동 | 한국 10 카테고리 표준 주기 매트릭스 + 리텐션 cohort 분석 가이드 |
| `commerce-product-image-pipeline` | 상품 이미지·영상 풀스택 파이프라인 오케스트레이터 — character-mgmt → image-gen(Soul) → video-gen(DOP) → media-channel-ad-packager 4단계 체인 자동 호출 | 이미지 5축(Hero·Lifestyle·Detail·Use-case·Result) + 영상 모션 프리셋 자동 + 채널 변환 + 비용 ₩2,300~4,000/상품 |

### LTV/CAC·푸시·법규 준수 (3)

| 스킬 | 역할 | 대표 출력 |
|---|---|---|
| `commerce-ltv-cac-architect` | 고객 단위 수익 구조 설계 — CAC→재구매율→구매주기→ARPU→공헌이익→LTV 6대 지표 + LTV/CAC ratio(<1 손실, 3~5 건강, ≥5 우수) + Payback Period + 광고 의존도 진단(30%+ 위험 → 11~15% 정상) | 손익분기 ROAS 자동 + 채널·세그먼트 분해 + 광고 의존도 탈출 6단계 로드맵 + 한국 D2C 카테고리 벤치마크 |
| `commerce-push-planner` | 앱 푸시 전용 기획 — 4원칙(왜/언제/누구에게/어떻게) + Timely·Personal·Actionable 3요소 + 카피 변형 3안(오늘만 vs 매일 / 누구나 vs 너에게만 / 숫자 vs 게이미피케이션 vs 브랜딩) | 한국 30+ 브랜드 레퍼런스(토스·배민·오늘의집·쿠팡·에이블리·지그재그·인프런·야놀자·퍼블리·넷플릭스·듀오링고) + 클릭률 예측 |
| `commerce-marketing-compliance-kr` | 한국 정통망법 광고·정보성 메시지 자동 게이트 — 6대 점검(광고성 판정·옵트인·야간 21시~익일 8시 차단·(광고) 표기·무료 수신거부·발신자 정보) | BLOCK/PASS 판정 + 위반 조항(제50조 1·3·4항, 제76조) + 채널별 베스트/워스트 카탈로그. 과태료 1회 최대 3,000만원 회피 |

### 광고·마진·자동화 (3)

| 스킬 | 역할 | 대표 출력 |
|---|---|---|
| `coupang-ad-optimizer` | 쿠팡 광고 풀세트 최적화 — 3 캠페인 분류(검색/비검색)·엔드 ROAS·자동규칙 3종 | 쿠팡 광고 월 매출 분석 + 최적화 로드맵 + 자동규칙 설정 가이드 |
...
| 스킬 | V6 매핑 | 대표 출력 |
|---|---|---|
| `commerce-market-research` | ① 시장조사 (4 MCP wrapper) | 거시·경쟁·검색 3축 시장 리포트 1장 + 포지셔닝 5축·새 카테고리 창출 vs 경쟁 |
| `commerce-jtbd-persona` | ② 고객분석 (3 MCP wrapper) | `--mode jtbd`: JTBD 9개 + 심리적 필요 4종 / `--mode persona`: 페르소나 3명 8필드 + 타겟 온도 4단계 메타 |
| `detail-page-copy` | ③ 상세페이지 | 기본 13섹션 + `--mode diagnose`: 7단계 진단 / `--mode copy`: 페르소나 2세트 (비율 25/50/25) + 혜택 언어 3단계 변환법 |
| `commerce-product-naming` | ④ 상품명 (3 MCP wrapper) | 공식 4요소[브랜드+카테고리+키워드+차별점] + 매핑 3안 카테고리 + 금지 키워드 9종 + 주의사항 4종 |
| `commerce-channel-message` | ⑤ 채널별 메시지 (2 MCP wrapper) | 6 심리 방아쇠 + 채널별 심리 상태 매트릭스 + 인지 편향 9종 + NCM(Need→Channel→Moment→Message→CTA) 검색·광고·CRM × 5종 = 15종 |
| `commerce-integrated-strategy` | ⑥ 통합 전략 (3 MCP wrapper) | 자동화 4단계 + 3 Phase 로드맵 + HITL Golden Rule + 매출 향상 전략 1장 + 실행 우선순위 Top 3 |
...
| `commerce-local-inventory-search` | 다이소·GS25·CU·세븐일레븐·이마트24·롯데마트·올리브영 상품·매장·재고 조회 | `daiso` CLI와 공개 MCP endpoint 기반 조회 결과 요약 |
| `campaign-planner` | 광고 캠페인 + 광고 심리학 완전판(성과 공식·3 동기·6 방아쇠·9 편향·PAS·후크 6종·영상 30초·타겟 온도×3동기 매트릭스·1st Party·CAC/LTV·단계별 예산 배분 80:20→65:35→50:50) | 캠페인 전략 + 심리 분석 |
| `sns-content` | SNS 콘텐츠 + 채널별 심리 상태 매트릭스 9채널 + 메타 학습 기간 가이드(48~72시간 절대 미수정) + iOS 14.5+ 후 CAPI 필수·GA4 교차 검증 | 채널별 카피 + 심리 분석 |
...
**3일 캠프 풀 체인 (V6 6도구 + Day 3 광고 풀세트)**
...
```text
> 강남역 다이소 수납박스 재고 찾아줘.
  - 주변 매장 후보와 재고 확인 결과를 같이 요약해줘
```

## `commerce-local-inventory-search`: 로컬 커머스 재고 조회

다이소, GS25, CU, 세븐일레븐, 이마트24, 롯데마트, 올리브영의 상품·매장·재고 조회를 다룹니다. upstream `hmmhmmhm/daiso-mcp`와 `daiso` CLI를 사용하며, 구매·예약·결제·로그인 자동화는 범위에 넣지 않습니다.
