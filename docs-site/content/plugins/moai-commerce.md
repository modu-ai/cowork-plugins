...
쇼핑몰을 운영하다 보면 상세페이지 카피, 쿠팡 광고 최적화, 리뷰 수집, 계절 캠페인 기획, 법규 점검까지 매일 다른 종류의 작업이 쌓입니다. `moai-commerce`는 월 매출 100만-10억 규모의 스마트스토어·자사몰·오픈마켓 셀러가 실제로 마주치는 그 작업 전부를 35개 스킬로 커버합니다.

상세페이지는 13섹션 감정여정 카피(Hero→Pain→Problem→Story→Solution→How→Proof→Authority→Benefits→Risk→Compare→Filter→CTA)와 1080×12720 단일 PNG 합성까지 자동화합니다. 시장조사·JTBD·페르소나 분석, 채널별 메시지 설계, 통합 전략 수립은 시장·고객·전략 분석 스킬군이 담당합니다. `commerce-morning-brief`는 매일 아침 주문·문의·트렌드·ROAS 4영역을 한 줄로 요약해 주고, `commerce-order-summary`는 스마트스토어·카페24·아임웹 채널을 통합해 신규 주문을 정리합니다.

판매 채널은 쿠팡·네이버 스마트스토어/11번가/G마켓/옥션, 카페24/아임웹/메이크샵 자사몰, 와디즈/텀블벅 크라우드펀딩, 카카오 메이커스/무신사/29CM 큐레이션, 네이버/카카오/그립/쿠팡 라이브 커머스 5개 유형을 커버합니다. 광고 수익 구조는 쿠팡 광고 풀세트 최적화, 마진·엔드 ROAS 자동 계산, CAC→LTV 6대 지표 설계, 광고 의존도 30%→11-15% 6개월 로드맵으로 구성됩니다. 정통망법 광고·정보성 메시지 자동 게이트로 과태료를 예방하고, 재구매 골든타임 3구간을 기반으로 CRM 발송을 설계합니다. 이미지 생성은 `moai-media:nano-banana`(Gemini 3 Image)·`media-gpt-image2-builder`(GPT Image 2)로 위임하며, 합성은 Pillow 기반 자체 스크립트로 처리합니다. 모든 텍스트 산출물은 `ai-slop-reviewer`로 자동 체이닝됩니다.
