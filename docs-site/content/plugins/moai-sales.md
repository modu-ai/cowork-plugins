...
RFP가 들어오면 영업팀은 고객 요구사항을 하나하나 대조하며 12섹션짜리 제안서를 처음부터 작성합니다. 항목을 빠뜨리면 평가에서 탈락하고, 차별점이 모호하면 가격 경쟁으로 빠집니다. 이 반복 작업이 영업 사이클 전체에서 가장 많은 시간을 잡아먹는 단계 중 하나입니다.

`moai-sales`는 이 한국 B2B SaaS·솔루션 회사의 영업 사이클을 압축합니다. RFP·고객 요구사항을 입력하면 12섹션 표준 목차와 Three C's(Compliant·Complete·Compelling) 원칙에 맞춰 제안서 초안을 자동으로 생성하고, 컴플라이언스 체크리스트로 누락 항목을 즉시 확인합니다. 견적서·세금계산서는 국세청 표준 양식을 따릅니다.

`kr-gov-grant`(moai-business)는 정부 지원사업 신청서, `investor-relations`(moai-business)는 투자자 IR 자료를 담당합니다. 본 플러그인은 **B2B 영업 고객 대상**으로 특화되어 있습니다.
