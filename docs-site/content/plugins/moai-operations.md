...
운영팀은 매주, 매월 비슷한 문서를 반복해서 만듭니다. 신규 프로세스마다 SOP를 작성하고, 구매 건마다 요청서와 발주서를 채우고, 회의가 끝나면 회의록을 정리합니다. 월말에는 주간·월간 보고서, 분기마다 OKR 현황과 리스크 매트릭스까지 더해집니다. 새 공급업체가 생기면 평가표와 계약 관리 문서도 따로 만들어야 하죠.

`moai-operations`는 이 운영 조직의 반복 문서 작성을 자동화합니다. SOP, 구매 요청서·발주서, 회의록, 주간·월간·분기 보고서, OKR 현황, 리스크 매트릭스, 벤더 평가·계약 관리까지 3개 스킬이 커버합니다.
