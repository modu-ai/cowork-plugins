...
AI 이미지 생성 도구를 쓸 때 가장 까다로운 부분은 막상 프롬프트를 쓰는 일입니다. GPT-image-2, Gemini 3 Pro Image, Midjourney는 각각 구조와 파라미터가 달라서 같은 아이디어라도 모델마다 다르게 표현해야 합니다. 공식 가이드를 참고해도 정확한 형식으로 변환하는 데 시간이 걸립니다.

`moai-media`는 두 가지 작업에 집중합니다. 첫 번째는 **이미지 프롬프트 텍스트 작성**입니다. 모델별 공식 가이드(OpenAI Cookbook·Google AI Developers·Midjourney Parameter List)를 그대로 적용해 ChatGPT·Google AI Studio·Discord `/imagine`에서 바로 복붙할 수 있는 프롬프트를 출력합니다. 두 번째는 **음성 합성**으로, ElevenLabs MCP로 TTS·보이스 클로닝·다국어 더빙·효과음을 생성합니다.
...
자연어 한 줄을 입력하면 AskUserQuestion 프리셋(제품샷·인물·일러스트·풍경)으로 컨텍스트를 추가 수집한 뒤, 각 모델의 공식 가이드에 정렬된 프롬프트 텍스트를 출력합니다. 책임 경계는 명확합니다. **프롬프트 텍스트 산출 전용**이며 실제 이미지 생성은 사용자가 외부 도구에서 직접 실행합니다.
