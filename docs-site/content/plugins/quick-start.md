...
---
플러그인을 처음 설치하면 '어디서 시작하지?'라는 감이 안 잡히기 쉽습니다. 순서는 단순합니다. 마켓플레이스를 Cowork에 등록하고, `moai-core`를 먼저 설치한 다음, 프로젝트를 초기화하면 됩니다. 이 페이지는 그 흐름을 처음부터 끝까지 따라갈 수 있도록 정리했습니다. 약 **10분**이면 첫 스킬 체인을 실행할 수 있습니다.

## 사전 체크

시작 전에 [Cowork 설치](../../cowork/install/)가 완료되어 있어야 하고, 산출물을 저장할 **로컬 폴더** 하나를 준비해 두세요. Windows에서는 경로가 길면 파일 생성에 실패할 수 있으니 짧은 경로를 권장합니다.
...
3. **도메인 플러그인 선택**
   ...
   지금 진행할 작업에 맞는 플러그인을 추가합니다. 사업계획서라면 `moai-business`와 `moai-office`를, 블로그 발행이라면 `moai-content`와 `moai-media`를, 계약서 검토라면 `moai-legal`과 `moai-office`를 선택하는 식입니다. 이미지를 생성할 때는 `moai-media`와 함께 `GEMINI_API_KEY`가 필요합니다. 23개를 한꺼번에 설치할 필요는 없습니다. 지금 당장 필요한 것만 골라 설치하세요.

4. **프로젝트 생성 및 `/project init`**

   Cowork 좌측 사이드바 **프로젝트** 섹션에서 **+ 새 프로젝트**를 눌러 프로젝트를 만들고, 프로젝트 설정 화면의 **작업 폴더 연결** 항목에 앞서 준비한 로컬 폴더를 지정합니다. 프로젝트·폴더 개념이 낯설다면 [프로젝트와 메모리](../../cowork/projects-memory/) 페이지를 먼저 읽어 보세요. 이후 대화창에 다음을 입력합니다.
...
5. **첫 요청**
   ...
   `/project init`이 끝나면 자연어로 원하는 것을 말하기만 하면 됩니다. `moai-core`의 라우터가 문맥을 파악해 적합한 스킬을 자동으로 호출합니다.
...
6. **산출물 확인**
   ...
   PPTX 파일이 작업 폴더에 저장되고, 대화창에는 **진단 → 수정 → 주요 변경사항** 3블록으로 구성된 AI 슬롭 검수 리포트가 함께 표시됩니다.

## API 키·커넥터 등록 (선택)

대부분의 스킬은 별도 키 없이 바로 사용할 수 있습니다. 이미지 생성·공공데이터 조회·CMS 자동 업로드처럼 외부 서비스와 연동하는 스킬에만 아래 키가 필요합니다.
...
키는 프로젝트 루트의 `.moai/credentials.env`에 저장됩니다. 외부 저장소에 커밋하지 마세요.

## 잘 안 될 때

스킬이 자동으로 호출되지 않는다면, `moai-core`가 설치되어 있는지, `/project init`이 실행됐는지부터 확인하세요. Word·PPT 파일이 깨지는 경우에는 `moai-office`와 함께 Python 의존성(`python-docx`, `python-hwpx` 등)이 설치되어 있는지 확인합니다. AI 슬롭 검수가 실행되지 않는다면 요청에 "빠르게"라는 표현이 있을 때 검수가 스킵될 수 있으니, "검수까지 돌려줘"라고 명시해 보세요.
