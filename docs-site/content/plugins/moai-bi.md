...
20페이지짜리 마케팅 보고서를 임원이 실제로 읽을 리 없습니다. 결재 라인은 핵심 수치와 의사결정 옵션을 1페이지에서 찾습니다. `moai-bi`는 그 1페이지를 자동으로 만드는 한국 임원·이사회 표준 BI 리포트 풀스택입니다.

복잡한 분석·재무·운영 보고를 ≤500단어로 압축하고, 단순 요약에 그치지 않고 **What / So What / Now What** 3축 구조로 의사결정 옵션과 권고안까지 담습니다. 재무 지표는 매출·영업이익률·EBITDA·운전자본 등 K-IFRS 기준을 우선하며, KOSIS·DART·공공데이터포털 데이터를 직접 인용할 수 있습니다. 기본 출력은 단일 HTML 파일로, 카톡이나 이메일에 그대로 첨부해 결재 라인과 즉시 공유할 수 있습니다.

## 기본 출력 = 단일 HTML 파일 (카톡 즉시 확인)

이 플러그인의 **모든 보고서는 기본적으로 `moai-content:html-report`로 단일 HTML 파일**을 만듭니다.
...
임원·이사회 보고 어조에 맞춰 `…합니다` 종결의 격식체를 기본으로 사용합니다. 모든 주장에는 수치와 출처를 결합하고 서술형 형용사는 최소화합니다. 단순한 상황 요약에 그치지 않고, A/B/C 옵션과 각 옵션의 리스크·기대효과를 명시합니다. 재무 지표는 K-IFRS·DART 기준으로 표기하며, 가독성을 위해 Pretendard 등 한국어 폰트를 HTML 안에 인라인으로 결합합니다.
