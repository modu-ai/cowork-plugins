...
## 왜 '하나의 HTML 파일'이 특별한가

흔히 보고서라 하면 '문서 파일'과 '거기에 들어간 이미지·글꼴·표'가 제각각 떨어져 나옵니다. 보내는 쪽은 폴더째 압축하고, 받는 쪽은 압축을 풀어야 글꼴이 깨지지 않는지 확인합니다. 한 번이라도 경로가 꼬이면 이미지가 빈 네모로 보이거나 표가 밀려버립니다. 우편에 비유하면, 보통 보고서는 내용물·사진·엽서·봉투가 따로 노는 '여러 봉투 묶음'입니다.

이 플러그인은 모든 것을 **봉인된 하나의 봉투(단일 HTML 파일)** 안에 넣습니다. 여기서 단일 HTML이란 글·그림·꾸밈 요소(CSS)·동작 스크립트(JS)를 전부 한 개의 `.html` 파일 안에 끼워 넣은 형태를 말합니다. 외부 파일을 아예 끌어오지 않으므로(한국어 폰트 CDN 한 건만 예외) 받는 사람이 어떤 기기·어떤 브라우저에서 열든 깨짐 없이 똑같이 보입니다. 봉투 하나만 카톡이나 이메일에 붙여 넣으면 즉시 결재 라인에 도착합니다. 여러 개를 챙기고 압축을 풀 필요가 없습니다.

```mermaid
flowchart LR
    A["재무·운영 데이터"] --> B["executive-summary<br/>1페이지로 압축"]
    B --> C["단일 HTML 봉투<br/>(글·그림·CSS·JS 인라인)"]
    C --> D["카톡·이메일 첨부"]
    D --> E["받는 사람<br/>어떤 기기에서든 그대로 열림"]
...
### What / So What / Now What — 증상에서 처방까지 한 장에 담기

`executive-summary`는 내용을 단순히 줄여 나열하는 게 아니라 **세 단계의 의사결정 파이프라인**으로 구성합니다. 의사 진료에 비유하면 이해가 쉽습니다. **What**은 '증상 관찰'입니다 — 무슨 일이 있었는지, 수치가 어땠는지 사실만 보고합니다. **So What**은 '진단'입니다 — 그 사실이 왜 중요한지, 무엇을 의미하는지 읽어냅니다. **Now What**은 '처방'입니다 — 그래서 지금 어떤 선택지(A/B/C)가 있고, 무엇을 권하는지를 밝힙니다.

일반 '줄인 요약'이 증상 나열에서 끝난다면, 이 3축 구조는 진단과 처방까지 한 장에 담습니다. 임원은 문서를 다 읽은 뒤에야 결정을 고민하는 게 아니라, Now What 단계에 놓인 권고안을 보고 바로 결재 버튼을 누를 수 있습니다. 이것이 '단순 요약'과 '임원용 요약'을 가르는 핵심 차이입니다.

```mermaid
flowchart LR
    W["① What<br/>무슨 일이 있었나<br/>(증상·수치)"] --> S["② So What<br/>무엇을 의미하나<br/>(진단·해석)"]
    S --> N["③ Now What<br/>무엇을 해야 하나<br/>(처방·권고)"]
