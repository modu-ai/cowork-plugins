...
기능 하나를 개발하기까지 PM이 작성해야 할 문서의 종류는 생각보다 많습니다. 유저 리서치부터 페르소나, PRD, 기능 명세, 로드맵까지 순서대로 챙겨야 하고, 분기가 바뀌면 간트 차트와 마일스톤을 다시 정리해야 합니다. VOC나 NPS 분석 결과를 개선 우선순위로 연결하는 작업도 반복됩니다.
...
사용자 인터페이스 디자인이 필요하거나 프로토타입을 빠르게 만들어야 할 때, 기존 사용자 경험을 개선하고 싶을 때, 또는 디자인 시스템이나 가이드라인을 새로 구축해야 할 때 사용합니다.

#### 준비물

사용자 리서치 결과, 사용자 흐름도(User Flow), 경쟁사 UI/UX 분석 자료, 브랜드 가이드라인을 미리 준비해 두면 더 정확한 결과를 얻을 수 있습니다.
...
웹·모바일에 최적화된 UI 디자인과 인터랙티브 프로토타입을 생성하고, 디자인 시스템 컴포넌트와 사용성 가이드라인 문서를 함께 제공합니다. 접근성 및 웹 표준 준수 여부도 확인합니다.
