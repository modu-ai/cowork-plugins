...
리파인먼트 도구는 목적에 따라 구분해서 쓰는 것이 효율적입니다. 전체 방향 전환이나 구조 변경은 자연어 수정이 가장 강하고, 특정 요소를 콕 집어 수정할 때는 인라인 코멘트가 정밀합니다. 카피·라벨을 바로 손보고 싶다면 캔버스에서 직접 텍스트를 편집하는 것이 가장 빠르고, 간격·색·레이아웃을 즉각적으로 조정하려면 조정 노브(sliders)를 활용하세요. 조정 결과는 현재 화면에만 적용하거나 모든 화면에 일괄 적용할 수 있습니다. 일관성을 위해 일괄 적용을 권장합니다.

| 조작 | 사용 시점 | 효율 |
|---|---|---|
| **자연어 수정** | 전체 방향 전환 · 구조 변경 | 큰 변화에 강함 |
| **인라인 코멘트** | 특정 요소를 콕 집어 수정 | 정밀 수정에 강함 |
| **직접 텍스트 편집** | 카피·라벨을 바로 손봄 | 카피 수정에 가장 빠름 |
| **조정 노브 (sliders)** | 간격·색·레이아웃을 live tweak | 시각 미세조정 |
...
캔버스 위 텍스트를 더블클릭하면 인플레이스 편집이 됩니다. 오타 수정·단어 교체·한 문장 다듬기처럼 단순한 카피 수정은 직접 편집이 가장 빠릅니다. 카피 전체를 재작성하거나 톤을 바꾸거나 다국어로 변환할 때는 채팅을 사용하세요.

## 조정 노브 — 미세 조정

화면 우측 또는 컨텍스추얼 패널의 슬라이더로 간격(spacing)·색(color)·레이아웃(layout)·모서리(radius)·그림자(elevation)를 즉시 조정합니다. 노브는 실시간으로 시각 피드백을 주기 때문에 5초 안에 다양한 조합을 시도해 보고 마음에 드는 값에서 멈추면 됩니다.

## 컨텍스트 누적 — 한 프로젝트 내에서

같은 프로젝트 안에서는 Claude가 이전 결과를 자연스럽게 상속합니다. "온보딩 디자인했던 이 스타일 그대로 설정 페이지에"처럼 짧은 지시 하나로 일관성이 유지되는 이유입니다. 권장 순서는 다음과 같습니다.
...
각 단계는 앞 단계의 결과를 자연스럽게 활용합니다.

## AI 슬롭 회피 프롬프트 — 첫 프롬프트에 항상 포함

학습 데이터의 평균값에서 벗어나려면 명시적으로 회피 패턴을 선언하는 것이 효과적입니다. Claude Opus 4.7부터는 이 회피 지시가 짧아도 충분히 잘 작동합니다.
...
프롬프트 첫 줄에 구체적인 직업·연차·소속을 가진 디자이너 역할을 부여하면 결과 일관성이 크게 올라갑니다. "UX 디자이너"보다 아래처럼 구체화할수록 효과적입니다.
...
여러 변화를 한 번에 요청하면 결과가 흐트러지고, 어디서 어긋났는지 파악하기 어려워집니다.
...
리파인먼트는 다섯 번째 라운드부터 효용 체감이 시작됩니다. 그 시점이 되면 `save this version` 명령으로 현재 결과를 저장하고, Canva·HTML·Code로 내보내 다른 도구에서 마감하세요. 그래도 부족하다면 새 프로젝트로 완전히 다시 시작하되, 그동안 배운 것을 첫 프롬프트에 반영하면 훨씬 나은 결과가 나옵니다.
