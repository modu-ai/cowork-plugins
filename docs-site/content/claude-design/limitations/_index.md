...
Claude·Cowork 데스크톱 앱과 모바일 앱에서는 작동하지 않습니다. 브라우저(Chrome·Safari·Firefox·Edge 최신)에서만 사용할 수 있어 오프라인·이동 중 작업이 불가하고, 데스크톱 앱 사용자는 브라우저를 별도로 실행해야 합니다. 결과물은 PDF·PPTX·HTML로 내보내 모바일에서 확인할 수 있습니다.

### 2. 큰 코드베이스 ingestion 지연

모노레포 전체나 enterprise 규모 코드베이스를 연결하면 디자인 시스템 추출 과정에서 모든 파일을 스캔하기 때문에 5분 이상 지연될 수 있습니다. UI 패키지 디렉토리만 연결하고(예: `packages/ui`), `.git`·`node_modules`·`dist`·`build`가 제외되어 있는지 확인하세요. DESIGN.md를 Claude Code에서 미리 합성한 뒤 업로드하면 비용과 시간을 크게 줄일 수 있습니다.

### 3. Research Preview의 점진 롤아웃

Research Preview 단계이기 때문에 가입자 모두에게 즉시 활성화되지 않습니다.
...
가입 직후 보이지 않더라도 며칠 기다리거나 Anthropic 지원에 문의하세요.
...
Figma는 디자이너 협업·실시간 공동 작업·정교한 디자인 시스템 운영(variables, components, libraries)·디자인 ↔ 코드 양방향(Dev mode)·플러그인 생태계·외부 공개 링크·전문 디자이너 워크플로우(vector edit·constraints·prototyping) 등에서 여전히 강합니다. Figma는 UI/UX 디자인 시장의 80-90%를 차지하며 디자인 시스템 운영의 표준입니다. Claude Design은 디자이너가 아닌 사람도 시각 자료를 만들 수 있게 하는 **저변 확대 도구**로 위치합니다.

### 6. 디자인 ↔ 코드 양방향 약함

디자인 → 코드 방향(Claude Code 핸드오프 번들)은 강하지만, 반대 방향인 코드 수정 후 디자인 동기화는 현재 제한적입니다. 코드에서 빌드한 결과를 스크린샷으로 캡처해 Claude Design에 다시 입력하는 방식으로 우회할 수 있습니다.

### 7. 사용량 추적·감사 로그 미제공

멤버별 사용량 대시보드·감사 로그·프로젝트별 토큰 사용량·사용량 알림 모두 현재 제공되지 않거나 제한적입니다. 금융·정부처럼 엄격한 거버넌스가 요구되는 조직은 도입 전에 이 점을 반드시 고려하세요.

### 8. 데이터 거주지 미지원

EU·한국 등 지역 데이터 거주지 요구가 있는 산업에는 적합하지 않습니다. 개인정보·금융·의료 데이터는 업로드 자체를 피해야 하며, Enterprise DPA 서명은 가능하지만 데이터 거주지 보장은 아닙니다. 자세한 데이터 처리 정책은 Anthropic Trust Center 또는 Sales에 문의하세요.

### 9. PPTX 변환의 한계

PPTX로 내보내면 캔버스 HTML 버전보다 레이아웃이 간소화됩니다. 배경 그라데이션·복잡한 일러스트·인터랙티브 요소가 특히 단순화되거나 누락될 수 있습니다. 정밀한 발표 자료가 필요하다면 PDF와 HTML을 함께 받아 비교한 뒤 더 나은 것을 선택하세요.
...
Anthropic 출시 공지에서 직접 명시한 단기 우선순위입니다.
...
이 한 줄은 Slack·Notion·Figma·Linear 같은 외부 도구로 Claude Design 결과물·핸드오프 번들을 자동으로 흘릴 수 있는 빌더 도구가 곧 제공될 가능성을 시사합니다. Cowork 사용자는 이 빌더가 나오면 기존 Cowork MCP 커넥터(Notion·Drive·Slack)와 결합해 풀스택 자동화가 가능해집니다.
