...
그룹 대화의 가장 큰 이점은 **결정 맥락이 자동으로 채팅 히스토리에 쌓인다**는 점입니다. "왜 사이드바 대신 탭으로 했는지" 같은 선택 근거가 남아, 핸드오프 시 Claude Code가 그대로 참고할 수 있습니다. PM의 비즈니스 우선순위·디자이너의 비주얼·엔지니어의 구현 제약이 같은 캔버스에서 충돌하고 합의되기 때문에, Figma·Slack·Notion 사이를 오갈 필요가 없습니다.

### 그룹 대화 운영 규칙

여러 명이 동시에 채팅하면 앞 메시지의 결과를 다음 메시지가 상속합니다. 충돌이 우려된다면 한 명씩 차례로 지시하세요. 큰 방향 전환은 소유자 또는 디자이너 책임자가 일괄 처리하고, 디테일 조정은 분산하는 것이 좋습니다. 새 멤버를 초대하면 채팅 히스토리도 함께 보이므로, 민감한 내용이 있다면 미리 정리하세요.

## 외부 공유 — 어떻게 하나

Claude Design 자체에는 외부 공개 링크가 없습니다. 외부 공유가 필요하다면 파일로 내보낸 후 전달하세요.
...
업로드하는 자산은 어떻게 저장되는지 알아두는 것이 중요합니다. Claude Design은 **Anthropic 엔터프라이즈 데이터 보존·삭제 정책**에 따라 자산을 저장합니다.
...
권한은 프로젝트 진행 상황에 맞춰 단계적으로 부여하는 것이 안전합니다.
...
Claude Design과 Cowork·Slack 사이에 직접 통합은 없지만, 내보낸 파일을 통해 결과물을 다음 경로로 흘릴 수 있습니다.
