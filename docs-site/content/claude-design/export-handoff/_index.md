...
같은 조직 멤버에게 링크만 보내면 되는 가장 빠른 공유 방법입니다. 인터랙티브 기능도 그대로 보존됩니다.
...
어디서나 열리고 폰트가 임베드된 정적 파일로, 외부 발송에 가장 안전한 형식입니다.
...
PowerPoint나 Keynote에서 추가로 편집해야 할 때 적합합니다. 다만 캔버스 HTML 버전보다 레이아웃이 간소화되는 점에 주의하세요.
...
인터랙티브 요소를 그대로 보존하면서 정적 호스팅이나 사내 인트라넷에 게시할 때 사용합니다. Vercel·Netlify·S3에 즉시 배포할 수 있는 자립 HTML 폴더로 내보냅니다.
...
핸드오프나 큰 변경 전에 전체 상태를 보존해 두는 백업·아카이브 용도입니다.
...
프로덕션 코드로 빌드할 때 사용하는 가장 강력한 형식입니다. 별도 섹션에서 상세히 다룹니다.

## Claude Code 핸드오프 번들 — 상세

Claude Design과 Claude Code는 같은 Anthropic 시스템에서 만들어졌기 때문에, 핸드오프 번들은 Claude Code가 픽셀에서 의도를 추론할 필요 없이 이미 구조화된 spec을 그대로 받는 형태로 작성됩니다.
...
핸드오프는 **디자인 → 코드** 방향이 강합니다. 반대 방향, 즉 코드를 수정한 뒤 디자인에 동기화하는 흐름은 현재 제한적입니다. 코드에서 빌드한 결과를 캡처해 Claude Design에 다시 입력하거나(스크린샷 피드백), 특정 페이지·섹션만 새 프로젝트로 재디자인한 뒤 부분 핸드오프하거나, 코드 변경이 클 때마다 디자인을 먼저 수정하고 다시 핸드오프하는 방식으로 우회할 수 있습니다.
