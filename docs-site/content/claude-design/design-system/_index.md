...
Claude Design은 기본값으로 비전 기반 모델(Claude Opus 4.7)이 학습한 일반적인 UI 패턴을 사용합니다. 아무것도 등록하지 않으면 수많은 AI 생성 디자인이 공유하는 평균값이 나옵니다. 반면 디자인 시스템을 등록하면 색 팔레트가 브랜드 정의로 고정되고(primary·secondary·accent + 100-900 스케일), 타이포그래피가 브랜드 폰트 패밀리·사이즈·웨이트로 맞춰지며, 컴포넌트가 실제 라이브러리(React·Vue·Tailwind)와 매칭되고, 레이아웃 패턴이 spacing·grid·페이지 구조로 잡힙니다. 한 번 Published 상태로 올려두면 모든 신규 프로젝트에 자동 적용되어 매번 다시 업로드할 필요가 없습니다. 핸드오프 시점에도 같은 시스템이 그대로 전달되므로 Claude Code가 기존 컴포넌트로 코드를 작성합니다.
...
가능한 한 많은 소스를 함께 올리는 것이 좋습니다. 한 가지만으로도 시작은 가능하지만, 여러 소스를 함께 주면 Claude가 훨씬 정확한 시스템을 추출합니다.
...
업로드가 끝나면 Claude가 5-15분 안에 색·타이포·컴포넌트·레이아웃·시맨틱 정보를 추출합니다. 각 항목을 브랜드와 대조해 어긋난 부분이 있으면 메모해 두고, 검증 프롬프트 단계에서 잡습니다.
...
발행(Publish) 전에 반드시 **테스트 프로젝트**를 만들어 일반 프롬프트를 실행합니다. 시스템의 약점은 실제 프롬프트를 던져 봐야 드러납니다.
...
한국 디자이너 환경에서 Figma는 가장 흔한 소스입니다. 단, Figma 파일의 품질이 추출 결과를 크게 좌우합니다. 권장 절차는 다음과 같습니다.
...
코드베이스 연결은 가장 강력한 시그널이지만, 모노레포 전체를 연결하면 ingestion이 수 분 이상 걸리고 시스템 인식도 어긋날 수 있습니다. UI 패키지 디렉토리만 연결하세요.
...
브랜드는 변합니다. 라이브 제품에서 색이나 폰트가 바뀌면 디자인 시스템도 함께 업데이트해야 결과물 일관성이 유지됩니다.
