...
Anthropic 공식 출시 공지(2026-04-17)에서 직접 인용된 도입 사례 2건입니다. 두 사례 모두 원칙 1(디자인 시스템부터)과 원칙 9(짧은 핸드오프 지시)를 정확히 활용한 결과입니다.
...
디자인 시스템 셋업 없이 프롬프트를 던지면 결과가 학습 데이터 평균값에 수렴합니다. "AI 티"가 나고, 핸드오프 시 컴포넌트 매핑도 안 됩니다. 반면 30분을 투자해 셋업해 두면 이후 모든 프로젝트에 자동으로 적용되어 결과물이 우리 브랜드처럼 보이고, 핸드오프 시에도 기존 코드 컴포넌트로 자동 매핑됩니다. 상세 절차는 [디자인 시스템 페이지](../design-system/) 참고.

## 원칙 2 — 역할을 명시한 프롬프트

프롬프트 첫 줄에 구체적인 직업·연차·소속을 가진 디자이너 역할을 부여하면 결과 분산이 30-40% 감소합니다. "UX 디자이너"처럼 막연하게 쓰지 말고 아래처럼 구체화하세요.
...
Claude가 기본값으로 선택하는 패턴(Inter·Roboto 폰트, 보라 그라데이션, 3-카드 그리드 등)은 수많은 AI 생성 디자인에서 반복되는 진부한 요소입니다. 첫 프롬프트에 아래 회피 블록을 포함하면 이를 벗어날 수 있습니다.
...
"미니멀하고 모던한 톤"이라는 문장보다 `linear.app/pricing` URL 하나가 훨씬 정확한 시그널을 줍니다. "신뢰감 있게"라는 표현 대신 자사의 잘 만든 페이지 스크린샷 2장을 올리고, "B2B SaaS 톤"이라고 쓰는 대신 `stripe.com`을 캡처해 함께 제공하세요. 업로드나 프롬프트에 URL을 직접 포함하는 습관을 들이면 결과 품질이 한 단계 올라갑니다.

## 원칙 6 — 토큰 비용 의식

Claude Design은 별도 쿼터를 사용하지만, 디자인 시스템을 직접 생성하거나 큰 코드베이스를 분석하면 토큰이 빠르게 소비됩니다. 아래 패턴으로 비용을 줄일 수 있습니다.
...
세부 사항을 길게 나열할수록 Claude Code가 자율적으로 추론할 여지가 줄어듭니다. 짧은 지시 하나로 핵심만 전달하고 나머지는 Claude Code가 코드베이스를 읽으며 스스로 파악하게 하세요.
...
한 번에 정적 UI와 인터랙션을 동시에 요청하면 결과가 흐트러집니다. 정적 시안을 먼저 확정한 다음, 별도 라운드로 마이크로 인터랙션을 추가하세요.
