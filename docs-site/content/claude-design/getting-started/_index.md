...
Claude Design 프로젝트는 다음 4가지 중 하나로 시작합니다. 처음이라면 **텍스트 프롬프트**가 가장 단순하고 빠릅니다.

### 1. 텍스트 프롬프트

빈 화면에 자연어로 원하는 결과를 설명합니다. 목적·청중·톤·컴포넌트 요구사항을 구체적으로 쓸수록 결과가 정확해집니다.
...
기존 문서나 이미지를 그대로 업로드해 시각 자료로 변환합니다. DOCX·PPTX·XLSX·PNG·JPG를 지원합니다.
...
이미 디자인 시스템이 코드에 구현되어 있다면 GitHub repo나 로컬 폴더를 연결하는 것이 가장 강력한 시작점입니다. Claude가 기존 컴포넌트·디자인 토큰을 자동으로 인식합니다.
...
운영 중인 웹사이트의 특정 요소를 직접 캡처해 새 디자인의 시작점으로 활용합니다. 본인 사이트 또는 권한이 있는 사이트만 사용하고, 헤더·푸터·카드·폼 같은 단위로 캡처한 뒤 "이 카드 디자인을 우리 브랜드 톤으로 다시"처럼 변형 프롬프트를 작성합니다.

## 첫 프롬프트 6요소 템플릿

비디자이너가 가장 자주 실패하는 지점은 "막연한 톤만 적는 것"입니다. "미니멀하고 모던하게" 한 줄만 적으면 Claude는 학습 데이터 평균값으로 결과를 냅니다. 다음 6요소를 채우면 결과가 한 단계 점프합니다.
...
처음 30분의 목표는 완성된 결과물이 아니라 **도구의 입력·출력 사이클 한 바퀴**를 경험하는 것입니다. 결과 품질을 본격적으로 끌어올리는 작업은 [디자인 시스템 설정](../design-system/) 페이지에서 시작됩니다.

## 자주 묻는 첫 질문

**Q. 일반 채팅 한도를 먹나요?**
Claude Design은 별도 쿼터를 사용합니다. 채팅·Claude Code 사용량과 완전히 분리되어 있어 서로 영향을 주지 않습니다.

**Q. 결과물 저작권은?**
사용자가 만든 결과물에 대한 권리는 사용자에게 있습니다. 다만 업로드한 자산이 제3자 권리를 침해하지 않도록 주의하세요.

**Q. 한국어 프롬프트가 가능한가요?**
네. 프롬프트와 결과물 카피 모두 한국어를 지원합니다. UI 자체는 일부 영문이 섞일 수 있습니다.

**Q. 다른 사람과 공유는 어떻게 하나요?**
같은 조직 안에서 링크 공유와 권한 3종(Private·View-only·Edit)을 사용합니다. 외부 공개 링크는 없습니다. 자세한 내용은 [협업·공유 페이지](../collaboration/)를 참고하세요.

**Q. Claude Code로 어떻게 넘기나요?**
Export 메뉴에서 "Hand off to Claude Code" 또는 "Hand off to Claude Code Web"을 선택합니다. 자세한 절차는 [내보내기·핸드오프 페이지](../export-handoff/)에서 확인할 수 있습니다.

## Cowork 플러그인 연계

6요소 템플릿을 매번 직접 채우는 것이 번거롭다면, v2.12.0부터 Cowork에 등록된 [`moai-design`](../../plugins/moai-design/) 플러그인을 활용하세요. Cowork 채팅에서 자연어로 호출하면 6요소를 자동으로 채워 줍니다.
