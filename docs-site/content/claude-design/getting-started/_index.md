...
```
Design a settings page for an admin tool with the following sections in
this order: Users, Integrations, Billing, Security, API keys. Use a clean
SaaS aesthetic with a left sidebar and a content area. Each section needs
a header, description, and primary action button. Dark mode supported.
```
