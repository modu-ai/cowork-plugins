...
### `drawio-diagram` — 편집 가능한 draw.io 다이어그램 렌더러 (moai-marketer 신규 스킬)
...
- **SKILL.md**: [GitHub](https://github.com/modu-ai/cowork-plugins/blob/main/moai-marketer/skills/drawio-diagram/SKILL.md)
- **온라인 문서**: [/moai-agents/marketer/](../../moai-agents/marketer/) (`drawio-diagram` 섹션)
- **영감 원본**: [Agents365-ai/drawio-skill](https://github.com/Agents365-ai/drawio-skill) (MIT) — MoAI 환경에 맞게 재구현(CLI 의존 제거, CDN 뷰어 전환)

{{< hint type="note" >}}
`moai-tutor:education-learning-material`은 이 스킬의 `.drawio` 도식을 학습자료에 ```drawio` 블록으로 조건부 임베드합니다(mermaid 보완용).
{{< /hint >}}
...
### `/project` agent-aware 강화 (moai-coworker)
...
- **스킬 카운트 173 → 177** — `drawio-diagram` 1개 신규(moai-marketer 14 → 15 스킬). 플러그인 28개 유지.
- **전체 버전 동기화 2.20.0 → 2.21.0** — marketplace.json + 28 plugin.json + 177 SKILL.md (206개 지점).
- **`moai-tutor:education-learning-material` drawio 연동** — ```drawio` 블록을 인식해 draw.io 뷰어를 조건부 임베드(`references/cdn-libraries.md` §6 신규). mermaid는 그대로 유지.
- 루트 README·docs-site 카탈로그에 drawio-diagram 추가, 배지(177 스킬)·플러그인 스킬 수 표 갱신.
- **기존 부채 정합 — 5개 플러그인 README 스킬 테이블** — 게이트 4(스킬 테이블 실측 일치) 28/28 플러그인 달성. moai-consultant(`ai-diagnostic` 추가), moai-seller(삭제 스킬 잔재 6행 제거 + `marketplace-coupang-ads` 추가), moai-tutor(`course-curriculum-design` rename stub 추가), moai-marketer(`meta-ads-manager` 추가), moai-media(`higgsfield-image`·`higgsfield-video` 추가). 각 플러그인 README 스킬 수 배지·도입부 카운트를 실측에 정합.
...
- 마켓플레이스 캐시를 갱신하면 `drawio-diagram`이 `moai-marketer`에 추가됩니다.
...
2. **플러그인 상세 재진입** — `moai-marketer`를 다시 열면 `drawio-diagram`이 스킬 목록에 나타납니다.
...
- **moai-marketer 플러그인 페이지**: [/moai-agents/marketer/](../../moai-agents/marketer/) (`drawio-diagram` · `humanize-korean` 섹션)
- **drawio-diagram SKILL.md**: [GitHub](https://github.com/modu-ai/cowork-plugins/blob/main/moai-marketer/skills/drawio-diagram/SKILL.md)
