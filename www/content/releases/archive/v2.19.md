...
flowchart TB
   ...
   HB --> Result
   CC --> Result
   style HA fill:#e6e6e6,stroke:#757575,color:#09110f
   style HB fill:#e8f1ec,stroke:#265240,color:#09110f
...
**Track A**: `moai-writer:general-humanize-korean`을 upstream [epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai) v1.6.1 → v2.0.0으로 정렬했습니다. 한국 번역학계가 정립한 8유형 번역투 계보를 분류 체계에 통합하고, 신규 패턴 4종(A-16·A-18·A-19·E-7)과 post-editese 14메트릭 레이어를 추가했습니다. 22→24 테스트 전부 PASS.
...
- **텍스트 산출 마감**: 모든 텍스트 출력 체인은 `moai-coworker:ai-slop-reviewer → moai-marketer:humanize-korean`으로 종료
...
- **humanize-korean `tests/test_metrics.py` 경로 복구** — v1.6.1 verbatim 포팅 시 upstream 레이아웃(`.claude/skills/...`)이 cowork 레이아웃(`moai-marketer/skills/...`)으로 적응되지 않아 `import metrics` 실패하던 사전 결함 수정(22 테스트 전부 실행·통과).
...
```text
> 사업계획서 처음부터 끝까지 만들어줘
→ moai-consultant 코디네이터 → strategy-planner → docx-generator → ai-slop-reviewer → humanize-korean
```
