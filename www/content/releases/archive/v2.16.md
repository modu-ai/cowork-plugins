...
flowchart TB
   subgraph New["신규 3 플러그인 (v2.16.0) — 18 스킬"]
       W["moai-accountant (6)<br/>개인 재무·재테크"]
       P["moai-coworker (7)<br/>자기관리·생산성"]
       C["moai-coworker (5)<br/>직장 커뮤니케이션"]
   end
...
- **moai-wealth**(현재 [`moai-accountant`](../../moai-agents/accountant/)에 통합) — 직장인 개인 자산관리(재테크 로드맵·가계부·투자·보험·연말정산·경제지표). 당시 법인 세무 도메인과 역할 분리.
- **moai-productivity**(현재 [`moai-coworker`](../../moai-agents/coworker/)에 통합) — 자기관리·생산성(회고·목표·시간·습관·자기돌봄·노션·주간보고). 당시 팀 PM 도메인과 역할 분리.
- **moai-comms**(현재 [`moai-coworker`](../../moai-agents/coworker/)에 통합) — 직장 대인 커뮤니케이션(보고·회의·피드백·갈등·면담·협상). 공식 인사 [`moai-recruiter`](../../moai-agents/recruiter/)·문서 [`moai-marketer`](../../moai-agents/marketer/)와 역할 분리.

마켓플레이스 23 → **26 플러그인**, 152 → **170 스킬**. 동기화 지점 176 → **198**. Breaking change 없음.

## What's New

### moai-accountant — 개인 재무·재테크 (6 스킬)

- **플러그인 페이지**: [/moai-agents/accountant/](../../moai-agents/accountant/)
- **GitHub 디렉터리**: [moai-accountant](https://github.com/modu-ai/cowork-plugins/tree/main/moai-accountant)
...
### moai-coworker — 자기관리·생산성 (7 스킬)

- **플러그인 페이지**: [/moai-agents/coworker/](../../moai-agents/coworker/)
- **GitHub 디렉터리**: [moai-coworker](https://github.com/modu-ai/cowork-plugins/tree/main/moai-coworker)
...
### moai-coworker — 직장 커뮤니케이션·소프트스킬 (5 스킬)
...
→ moai-accountant/wealth-roadmap → 현황 진단 → 종잣돈 단계 → 자산 배분 → 자동화 로드맵
```

```text
> 신년 목표 세웠는데 12주 계획법으로 실천 가능하게 쪼개줘
→ moai-coworker/goal-planner → 12주 압축 → 주간 마일스톤 → 루틴 연결
```

```text
> 내일 팀장님한테 프로젝트 지연 보고해야 하는데 어떻게 말해야 깔끔할까?
→ moai-coworker/report-speak → 두괄식 결론 구조 → 상사 유형별 톤 → 30초 보고 스크립트
```

```text
> 연말정산 환급 더 받으려면 12월 전에 뭘 챙겨야 해?
→ moai-accountant/personal-tax-saver → 소득공제 vs 세액공제 → 항목별 공제 전략
```

## 관련 문서 & 출처

- **CHANGELOG**: [전체 변경 사항](https://github.com/modu-ai/cowork-plugins/blob/main/CHANGELOG.md)
- **moai-accountant 플러그인 페이지**: [/moai-agents/accountant/](../../moai-agents/accountant/)
- **moai-coworker 플러그인 페이지**: [/moai-agents/coworker/](../../moai-agents/coworker/)
- **moai-coworker 플러그인 페이지**: [/moai-agents/coworker/](../../moai-agents/coworker/)
