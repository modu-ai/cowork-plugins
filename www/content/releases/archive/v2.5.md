...
**Layer 3** `meta-ads-analyzer`는 메타 광고관리자에서 추출한 `.xlsx` 보고서 1-6개를 받아 9 분석 모듈(퍼널·KPI·차원·매트릭스·누수·라이프사이클·학습·예산·시뮬레이션) + 4D 교차 분석(광고×지면×연령×성별) + 3 사용자 그룹 톤(인하우스/대행사/소규모, 명시 입력) + 4 출력 형식(HTML/DOCX/PPTX/MD, cowork 공용 디자인 토큰 적용) + {{< icon name="circle" class="tone-success" >}}{{< icon name="circle" class="tone-warning" >}}{{< icon name="circle" class="tone-error" >}} 강도별 액션 옵션을 제시합니다. claude-ads의 50-check 매트릭스를 한국 시장에 맞춰 매핑하고 메타 광고관리자 UI 조작 가이드는 배제(REQ-META-ADS-018)해 단정적 명령이 아닌 의사결정 옵션을 제공합니다.
...
### moai-marketer 신규 1 스킬

**`meta-ads-analyzer`** — 메타 광고관리자 `.xlsx` 보고서 분석기

- **SKILL.md GitHub URL**: [moai-marketer/skills/meta-ads-analyzer/SKILL.md](https://github.com/modu-ai/cowork-plugins/blob/main/moai-marketer/skills/meta-ads-analyzer/SKILL.md)
- **문서 페이지**: [/moai-agents/marketer/](/moai-agents/marketer/)
...
- 강도별 액션 옵션: {{< icon name="circle" class="tone-success" >}} 보수안 / {{< icon name="circle" class="tone-warning" >}} 중도안 / {{< icon name="circle" class="tone-error" >}} 적극안 + 양면 해석 + 의사결정 가이드
...
- `moai-marketer/.mcp.json` 신규 — 2 MCP 서버 등록 (`meta-ads` hosted + `moai-ads-audit` local stdio)
- `moai-marketer/CONNECTORS.md` 신규 — `META_ACCESS_TOKEN` 발급 절차 + Layer 1 fallback 옵션 4종(Meta 공식 · Adspirer · byadsco · pipeboard) + 10 도구 명세

## Changed

- 마켓플레이스 스킬 카운트: 129 → **130** (+1 신규, mcp-server 1종 신규)
- 동기화 지점: 151 → **152** (marketplace 1 + plugin.json 21 + SKILL.md 130)
- `moai-marketer` plugin.json `description` v2.5.0 신규 항목 추가
- `marketplace.json` `plugins[]` 배열의 `moai-marketer` description 갱신
- 루트 README 배지(Version 2.5.0 · Skills 130) + v2.5.0 하이라이트 섹션
- `moai-marketer/README.md` 스킬 테이블 11행 + MCP 서버 섹션 신규
...
2. **`moai-marketer` 플러그인 상세 재진입** — 새 스킬 `meta-ads-analyzer`와 MCP 서버 2종이 자동 감지됩니다.
3. **Meta 공식 MCP 활성화 (선택)** — Layer 1 데이터 fetch를 위해 `META_ACCESS_TOKEN` 환경변수 등록. 발급 절차는 `moai-marketer/CONNECTORS.md` §Meta Ads 참조. 비활성 환경에서는 `.xlsx` 업로드 fallback이 자동 동작합니다.
...
- **moai-marketer 플러그인 페이지**: [/moai-agents/marketer/](/moai-agents/marketer/)
...
- **NOTICE attribution**: [NOTICE.md §agricidaniel/claude-ads (MIT)](https://github.com/modu-ai/cowork-plugins/blob/main/NOTICE.md)
- **MCP 등록 가이드**: [moai-marketer/CONNECTORS.md](https://github.com/modu-ai/cowork-plugins/blob/main/moai-marketer/CONNECTORS.md)
