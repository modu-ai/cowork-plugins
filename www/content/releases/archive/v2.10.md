...
weight: 900
description: "신규 플러그인 moai-writer — 한국 출판사 제출용 원고 풀스택 8 스킬. 컨셉서·페르소나·목차·저자 약력·제안서·출판사 매칭·본문·퇴고. KPIPA·국립국어원·도서정가제·30+ 한국 출판사 + 자비 출판 5 플랫폼. 21 → 22 플러그인, 147 → 155 스킬."
...
flowchart TD
   ...
   style I fill:#e8f1ec,stroke:#2a8a8c,color:#09110f
```

## Highlights

v2.10.0은 **"신규 플러그인 `moai-writer` — 한국 출판사 제출용 원고 풀스택 8 스킬"** 출시입니다. 도서 컨셉서부터 출판사 매칭·본문 집필·퇴고까지 **8 단계 워크플로우를 단일 플러그인**에 통합했습니다.
...
book-concept-planner
  ...
  → book-chapter-writer
  → book-revision-coach
  → moai-marketer:korean-spell-check
  → moai-marketer:humanize-korean       # AI 티 정밀 윤문 (필수)
  → moai-coworker:ai-slop-reviewer         # 최종 검수 (필수)
```
...
## 비교: moai-tutor vs moai-writer

`moai-tutor:education-grant-writer`나 `paper-writer`와 혼동하지 않게 정리합니다.

| 용도 | 권장 플러그인 |
|---|---|
| 정부지원사업 신청서 | `moai-consultant:business-kr-gov-grant` |
| 학술 논문 작성 | `moai-tutor:education-paper-writer` |
| **한국 출판사 제출용 단행본 원고** | **`moai-writer` (신규)** |
| 블로그·뉴스레터 | `moai-marketer:content-blog` / `newsletter` |

## Quality

- 8 스킬 4차원 루브릭 자가 평가: 가중 평균 **0.85** (통과 기준 0.70 {{< icon circle-check >}})
...
- `moai-writer` 사용 시 `moai-marketer`(맞춤법·humanize) + `moai-coworker`(ai-slop-reviewer) 함께 설치 권장
...
| 플러그인 매니페스트 | `<plugin>/.claude-plugin/plugin.json` | 22 (moai-writer 신규 1 추가) |
| 스킬 frontmatter | `<plugin>/skills/<skill>/SKILL.md` | 155 (moai-writer 신규 8 추가) |
...
# 이후 플러그인 상세 재진입 시 moai-writer 노출
# 신규 플러그인이므로 별도 + 버튼으로 활성화 필요
```

기존 워크플로우 그대로 동작합니다. moai-writer은 별도 활성화가 필요한 신규 플러그인입니다.

## 관련 문서 & 출처

- **moai-writer 플러그인 페이지**: [/moai-agents/writer/](/moai-agents/writer/)
- **GitHub 저장소**: [modu-ai/cowork-plugins/tree/main/moai-writer](https://github.com/modu-ai/cowork-plugins/tree/main/moai-writer)
