...
flowchart TB
   subgraph Day1["Day 1 셋업 (3)"]
       A1[moai-coworker:mcp-connector-setup]
   ...
   style A1 fill:#e6e6e6,stroke:#757575,color:#09110f
...
#### moai-coworker:mcp-connector-setup
...
- **링크**: [SKILL.md](https://github.com/modu-ai/cowork-plugins/blob/v2.3.0/moai-coworker/skills/mcp-connector-setup/SKILL.md) · [SPEC-CAMP-SETUP-005](https://github.com/modu-ai/cowork-plugins/blob/v2.3.0/.moai/specs/SPEC-CAMP-SETUP-005/spec.md)

#### moai-seller:commerce-morning-brief
- **한 줄 기능 요약**: MCP `dashboard_morning_brief` → 어제 주문·신규 문의·트렌드·ROAS **4영역 1줄 통합**
- 페어 분리: `moai-officer:office-daily-briefing`(외부 뉴스·웹) vs 본 스킬(MCP 매장 데이터)
- **링크**: [SKILL.md](https://github.com/modu-ai/cowork-plugins/blob/v2.3.0/moai-seller/skills/commerce-morning-brief/SKILL.md)

#### moai-seller:commerce-order-summary
- **한 줄 기능 요약**: MCP `order_summary_today` → 스마트스토어 + 카페24 + 아임웹 **채널 통합 신규 주문 1줄**
- **링크**: [SKILL.md](https://github.com/modu-ai/cowork-plugins/blob/v2.3.0/moai-seller/skills/commerce-order-summary/SKILL.md)

### Day 2 V6 6도구 (5 신규 + 1 강화)

V6(정승우님) 7교시 구조 1:1 매핑 wrapper. PDF §5.3 V6 ↔ MCP 매핑.
...
| 스킬 | 용도 | 출력 |
|------|------|------|
| `course-curriculum-design` | 3일 21세션 시간표 + 강사·조교 동선표 + D-7 사전 준비물 + 리스크 Plan B 5건+ | `moai-officer:office-docx-generator` 자동 체이닝 → Word(.docx) |
...
**통합 1건**: `content-social-media`(당시 moai-content) → `content-sns-content`(현재 `moai-marketer`)에 흡수
- 글로벌 4채널 모드 추가: **스레드·X·링크드인·유튜브 쇼츠**
- 한국 3채널 모드(인스타·네이버 블로그·카카오)는 유지
- `social-media`는 deprecate stub + 리디렉션 안내 (v2.5.0까지 호환)

**책임 분리 1건**: `moai-marketer:marketing-campaign-planner`
- "이커머스 상세페이지 제작·AI 이미지 생성" 책임 제거
- 상세페이지 카피 → `moai-seller:commerce-detail-page-copy`
- 상세페이지 합성 이미지 → `moai-seller:commerce-detail-page-image`
...
- 당시 moai-core·moai-content·moai-marketing·moai-education·moai-commerce·moai-media 6개 플러그인 description 갱신 (현재는 각각 moai-coworker·moai-marketer·moai-marketer·moai-tutor·moai-seller·moai-media)
...
또는 `moai-officer:office-mcp-connector-setup` 스킬을 호출해 통합 가이드를 받으세요.
...
**v2.4.0 출시 예정**: `cowork-plugins/mcp-servers/moai-seller/` monorepo에 Python(uvx) 기반 MCP 서버 구현. 광고 운영 4종(meta ads · tiktok ads · 네이버 광고 · 카카오 모먼트) + Phase 1 34종 + Higgsfield MCP 통합.
...
| 작업 | 사용할 스킬 |
|------|-------------|
| 일반 광고·마케팅 카피 | `moai-marketer:content-copywriting` |
| 이커머스 광고·톡톡·푸시 카피 | `moai-seller:commerce-copywriting` |
| 상세페이지 13섹션 카피 | `moai-seller:commerce-detail-page-copy` (강화) |
| 상세페이지 합성 이미지 | `moai-seller:commerce-detail-page-image` |
| 한글 타이포 광고 이미지 5장 | `moai-media:media-gpt-image2-builder` {{< icon sparkles >}} |
| 광고 영상 (카테고리 자동 라우팅) | `moai-media:media-model-router` {{< icon sparkles >}} |
| 한국 3채널 + 글로벌 4채널 SNS | `moai-marketer:content-sns-content` (확장) |
| 캠페인 단위 전술 (1-3개월) | `moai-marketer:marketing-campaign-planner` (책임 분리) |
| 사업·OKR·BMC 단위 전략 (1-5년) | `moai-consultant:business-strategy-planner` |
...
### 영향받은 플러그인 페이지
- [moai-seller 플러그인 페이지](/moai-agents/seller/) — V6 6도구 추가
- [moai-media 플러그인 페이지](/moai-agents/media/) — Day 3 광고 풀세트 추가
- [코워커 플러그인 페이지](/moai-agents/coworker/) — mcp-connector-setup 추가
- [moai-tutor 플러그인 페이지](/moai-agents/tutor/) — 3일 캠프 운영 매뉴얼 + 30일 후기 자산화 시퀀스 추가

### CHANGELOG
- [CHANGELOG.md (v2.3.0 섹션)](https://github.com/modu-ai/cowork-plugins/blob/main/CHANGELOG.md)

### 외부 출처
- 모두의 커머스 3일 마스터 캠프 PDF 기획서 (강사: 정승우)
...
**Track A MoAI-Commerce MCP 서버 구현** (`cowork-plugins/mcp-servers/moai-seller/` monorepo):
