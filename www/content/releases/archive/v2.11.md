...
```mermaid
flowchart TD
   A["개편 전 16 스킬<br/>(wrapper 12 + 빌더 3 + 음성 1)"] --> B["개편 후 4 스킬<br/>(빌더 3 + 음성 1)"]
   B --> C["이미지 생성<br/>Higgsfield MCP<br/>(단일 통합)"]
   B --> D["영상 생성<br/>Higgsfield MCP<br/>(단일 통합)"]
   B --> E["음성 생성<br/>ElevenLabs MCP<br/>(audio-gen 래퍼)"]
   style A fill:#f5dcd7,stroke:#c44a3a,color:#09110f
   style B fill:#e8f1ec,stroke:#2a8a8c,stroke-width:2px,color:#09110f
...
- **moai-media 16→4 축소** — 이미지·영상·음성 직접 호출 wrapper 12개를 제거. 이미지·영상은 **Higgsfield MCP 단일 통합**, 음성은 ElevenLabs MCP로 환원. 본 저장소는 **프롬프트 텍스트 빌더 + ElevenLabs 음성**으로 책임 경계를 명확화했습니다.
- **5 플러그인 페이지 재정의** — moai-seller · moai-media · moai-tutor · moai-analyst · moai-career 각 페이지의 책임 경계와 도메인 카탈로그를 다시 정렬했습니다.
...
**제거된 12 스킬** (Higgsfield MCP 단일 통합):

- `nano-banana`, `image-gen` — 이미지 생성은 **Higgsfield MCP** 단일 통합
- `video-gen`, `speech-video` — 영상·토킹헤드는 **Higgsfield MCP** 단일 통합
- `character-mgmt` — Higgsfield Character / LoRA는 Higgsfield MCP 직접 호출
- (중복 영상 게이트웨이 스킬) — **사용 중단** (Higgsfield MCP 단일 통합으로 환원)
...
| 플러그인 | 변경 후 |
|---|---|
| **moai-seller** | 35 스킬 도메인 카탈로그(시장조사·JTBD·페르소나·상품명·통합전략 등 9 카테고리)로 재작성 |
| **moai-media** | 이미지 프롬프트 빌더 3종 + audio-gen 4 스킬로 재작성. Higgsfield MCP 단일 통합 |
| **moai-tutor** | 강사·교수·교사 교육 콘텐츠 풀스택으로 재정의. `course-curriculum-design`은 1일~16주 모든 코스 형식 지원, `course-followup-sequence`는 코스 종료 후 후기 자산화로 일반화 |
| **moai-analyst** | `executive-summary` 산출물을 `html-report` 중심으로 재정의. 단일 HTML 파일에 이미지·CSS·JS 모두 인라인 + pdf/docx/pptx/hwpx 변환은 옵션 |
...
기존에 moai-media wrapper 스킬(`nano-banana`·`image-gen`·`video-gen` 등)을 자연어로 호출하던 워크플로우는 **Higgsfield MCP**로 환원해 직접 사용하면 됩니다.

| 기존 wrapper | 환원 후 |
|---|---|
| `nano-banana`, `image-gen` (이미지 생성) | **Higgsfield MCP** `higgsfield.soul.*` / ChatGPT(GPT-image-2) / Google AI Studio(Gemini 3 Pro Image) |
| `video-gen` (영상 생성) | **Higgsfield MCP** `higgsfield.dop.*` (Kling·Veo·Hailuo 등) |
| `speech-video` (토킹헤드) | **Higgsfield MCP** `higgsfield.speak.*` |
| `character-mgmt` (캐릭터 관리) | **Higgsfield MCP** `higgsfield.character.*` |
| (중복 영상 게이트웨이) | 사용 중단 (Higgsfield MCP 단일 통합으로 환원) |
| `media-*` wrapper | Higgsfield MCP + 프롬프트 빌더 3종 조합 |

번들 MCP 2종(`higgsfield` · `elevenlabs`)이 포함되어 사용자 측 추가 설치는 불필요합니다. **이미지·영상 직접 생성은 Higgsfield MCP 단일 채널로 통합**되어 다른 미디어 MCP는 사용하지 않습니다.
...
- **moai-media 플러그인 페이지**: [/moai-agents/media/](/moai-agents/media/)
- **moai-tutor 플러그인 페이지**: [/moai-agents/tutor/](/moai-agents/tutor/)
- **moai-career 플러그인 페이지**: [/moai-agents/career/](/moai-agents/career/)
- **moai-analyst 플러그인 페이지**: [/moai-agents/analyst/](/moai-agents/analyst/)
