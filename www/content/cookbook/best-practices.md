...
3. **지시사항 추가 (선택)** — 프로젝트별 맞춤 규칙을 AGENTS.md에 적거나 전역 지시사항에 저장합니다.
...
| 프로젝트 지침 파일 | 프로젝트·폴더 단위 | 정본은 `AGENTS.md` 한 파일. ChatGPT Work는 직접, Claude Cowork는 `CLAUDE.md`의 `@AGENTS.md` 연결고리를 통해 매 세션 시작 시 자동 로드. 영구 규칙 |
| Auto Memory | 프로젝트 내 | 사용자 선호·수정 이력을 자동 학습 |

{{< hint type="warning" >}}
전역 지시사항은 200줄을 넘기지 마세요. 길수록 토큰이 낭비되고 준수율이 떨어집니다. 프로젝트별 세부 규칙은 AGENTS.md로 내리세요.
{{< /hint >}}
...
`moai-cowork`는 이 패키지 모델을 따라 만들어진 공식 플러그인(다수 스킬) 저장소입니다.
...
동료에게 메신저를 남기는 상황을 떠올려 보세요. 부하코워커에게 "버튼을 누르고, B열을 복사하고, 새 시트를 열고, 합계를 구하고, 차트를 만들어줘"라며 단계를 일일이 지시하는 건 Chat 시절의 사고방식입니다(마이크로매니저). 반면 능력 있는 동료에게 "지출 범주별 Word 보고서를 만들어줘, 요약과 상위 5개 항목 표를 포함해서"라고 **결과만 남겨두고** 커피를 마시러 가는 것이 Cowork 시절의 사고방식입니다(위임). 핵심은 상대가 무엇을 원하는지만 알면 어떻게는 스스로 찾는다는 점입니다. Cowork는 그 능력 있는 동료입니다.
...
{{< icon name="circle-x" class="tone-error" >}} Chat 방식 — 단계를 일일이 지시합니다:

{{< terminal title="claude — cowork" raw="true" >}}
"파일을 열어 B열을 복사한 뒤 새 시트에 붙여넣고 합계를 구한 다음 차트를 만들어줘"
{{< /terminal >}}

{{< icon name="circle-check" class="tone-success" >}} Cowork 방식 — 결과만 말하면 됩니다:
...
[ ] 프로젝트 지침 파일 작성 (AGENTS.md 정본 + CLAUDE.md 연결고리, 프로젝트 규칙)
