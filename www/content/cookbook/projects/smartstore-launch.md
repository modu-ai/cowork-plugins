...
weight: 30
description: "상세페이지·홍보 콘텐츠·고객 응대까지 — 셀러, 마케터, CS매니저 세 코워커가 릴레이로 신제품 런칭을 완주합니다."
...
> **투입 코워커** — 셀러(`moai-seller`) → 마케터(`moai-marketer`) → CS매니저(`moai-cs`)
...
런칭은 사실 한 가지 일이 아니라 세 가지 일의 묶음입니다. **팔리게 만드는 일**(상세페이지), **알리는 일**(홍보 콘텐츠), **지키는 일**(고객 응대). 쇼핑몰 회사라면 MD·마케터·CS팀이 나눠 맡는 일을 지은 님은 혼자 다 하고 있었던 겁니다. 이번엔 세 명의 AI 코워커에게 각자의 몫을 맡깁니다.
...
단계 사이에 시간이 지나 맥락이 끊긴 경우입니다. 마케터 단계에서 "위 상세페이지 문구의 톤 그대로"라고 명시하거나 상세페이지 문구 파일을 다시 첨부해 기준을 고정하세요. 브랜드 말투 규칙(해요체, 금지 표현 등)을 한 줄로 정리해두면 어느 코워커에게든 재사용할 수 있습니다.
{{< /hint >}}
