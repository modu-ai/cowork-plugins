...
사무관의 `office-public-data-public-data`가 공공데이터포털·KOSIS 계열 데이터를 찾아 가져오는 입구입니다. 가져온 데이터는 `data-explorer`가 추세·비교 분석을 하고, `data-visualizer`가 보고서에 넣을 차트로 그립니다. 마지막 `doc-hwp`가 분석 내용과 차트를 공공기관 보고서 서식(개요–현황–분석–시사점)에 맞춰 HWPX 파일로 만들어냅니다. 쿡북 공통 원칙대로 숫자·차트는 어투 검수 대상이 아니므로, 이 체인엔 ai-slop-reviewer가 없습니다 — 대신 서술형 시사점 문단이 길어지면 그 부분만 검수를 붙이면 됩니다.
