...
title: "사용 패턴 가이드"
weight: 48
description: "Cowork는 '긴 프롬프트를 직접 쓰는 도구'가 아닙니다. 짧은 자연어 한 줄 → 시스템이 묻고 → 스킬 체이닝으로 자동 처리. 4가지 표준 패턴을 익히면 17개 도메인 213개 스킬을 자유롭게 조합할 수 있습니다."
...
## 핵심 원칙 {{< icon sparkles >}} 짧게 말하세요. 나머지는 시스템이 묻습니다.
...
### {{< icon circle-x >}} 잘못된 사용법

{{< hint type="warning" >}}
사용자가 매번 옵션·요구사항을 일일이 명시하는 것은 **권장 사용법이 아닙니다**. 그러면 시스템의 자동 인터뷰 기능과 스킬 체이닝이 무력화됩니다.
{{< /hint >}}

{{< terminal title="claude — cowork ({{< icon circle-x >}})" >}}
...
위 형식은 **사용자가 시스템 내부 구조(스킬명·체인 순서·산출 옵션)를 미리 알고 있어야** 가능합니다. 이는 모두의 코워크의 설계 의도와 정반대입니다.

### {{< icon circle-check >}} 올바른 사용법

{{< terminal title="claude — cowork ({{< icon circle-check >}})" >}}
...
### 패턴 1 {{< icon sparkles >}} 단일 프롬프트 실행 (Single Prompt)
...
### 패턴 2 {{< icon sparkles >}} 멀티턴 대화 (Multi-Turn Dialog)
...
### 패턴 3 {{< icon sparkles >}} 배치 처리 (Batch Processing)
...
**예시**: NDA 12개 일괄 검토 (앞서 본 {{< icon circle-check >}} 사례)
...
### 패턴 4 {{< icon sparkles >}} 스케줄 자동화 (Scheduled Automation)
...
flowchart TD
   ...
   style P2 fill:#e8f1ec,stroke:#265240
   style P3 fill:#f5dcd7,stroke:#c44a3a
   style P4 fill:#e8f1ec,stroke:#3a7c5f
```
...
- **[플러그인 카탈로그](../../plugins/)** — {{< catalog-count plugins >}} 도메인 {{< catalog-count skills >}} 스킬 전체 목록
