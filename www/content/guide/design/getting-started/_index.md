...
aliases: ["/claude-design/getting-started/"]
---
...
| 롤아웃 | Beta 단계, 점진 활성 — 가입 직후 즉시 보이지 않을 수 있음 |

![Claude Design 기본 레이아웃 — 왼쪽 대화(chat), 오른쪽 캔버스(canvas)](/screenshots/design/cd-access-layout.png)

Claude Design에 진입하면 화면이 두 영역으로 나뉩니다: 왼쪽은 대화 패널, 오른쪽은 디자인 캔버스입니다.
...
![Claude Desktop 사이드바에서 Claude Design 진입](/screenshots/design/cd-access-desktop.png)

Claude Desktop 앱의 사이드바에서 Claude Design 버튼을 눌러 시작합니다.
...
![텍스트 프롬프트 한 줄로 생성된 첫 인터랙티브 시안](/screenshots/design/cd-first-prompt.png)

텍스트 프롬프트를 입력하면 Claude가 즉시 첫 시안을 생성하기 시작합니다.
...
**Q. 일반 채팅 한도를 먹나요?**
A. 예, 먹습니다. Claude Design은 일반 채팅·Claude Code와 동일한 **공유 풀**에서 사용량을 차감합니다 (별도 할당량 없음). 따라서 Claude Design을 많이 쓰면 채팅 한도가 줄어듭니다. 한도를 초과하려면 Extra usage를 활성화해야 합니다.
