...
자체 제작 MCP 서버(`moai-` 로 시작하는 것들)를 쓰려면 준비물이 하나 필요합니다.
한 번만 설치하면 모든 MCP 서버가 이걸 씁니다.
...
발급받은 값은 설정 파일의 `env` 부분에 넣습니다. 예를 들어 유튜브는 이렇습니다.

```json
{
  "env": {
    "YOUTUBE_CLIENT_ID": "여기에 붙여넣기",
    "YOUTUBE_CLIENT_SECRET": "여기에 붙여넣기",
    "YOUTUBE_REFRESH_TOKEN": "여기에 붙여넣기"
  }
}
```
...
저장소에 커밋하지 마세요. 라이브 방송 중 화면 공유 사고가 가장 흔합니다.
{{< /hint >}}
