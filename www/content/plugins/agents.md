...
| 스킬 자동 매칭 | 동작 | 동작 |
| MCP 연동 | 동작 | 동작 |
| 슬래시 명령 (`/project` 등) | 동작 | **미지원** — 자연어로 대체 |
| 에이전트 명시 호출 | 동작 | **미지원** — 자연어로 대체 |
| worker/auditor 검수 루프 | 에이전트로 자동 분리 | 요청을 두 번 나눠서 수동 진행 |

### 슬래시 명령 ↔ 자연어 대응표

Codex에서는 아래 왼쪽 대신 오른쪽처럼 말하면 됩니다. 부르는 스킬이 같으니 결과도 같습니다.

| Claude Cowork | ChatGPT Work — 이렇게 말하세요 |
|---|---|
| `/project` | 새 프로젝트 시작해줘 — 지침이랑 에이전트 만들어줘 |
| `/brief` | 디자인 브리프 만들어줘 |
| `/design` | 디자인 작업 시작해줘 |
