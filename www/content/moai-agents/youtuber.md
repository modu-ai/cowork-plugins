...
유튜브에는 Google이 관리하는 공식 MCP 서버가 없습니다. 그래서 YouTube Data API v3와 Live Streaming API, Analytics API를 감싼 **자체 MCP 서버를 준비하고 있습니다.** 연결되면 방송 생성과 송출 상태 전환, 실시간 채팅 관리, 업로드와 메타데이터, 성과 조회, 댓글 처리를 대화 안에서 바로 할 수 있게 됩니다.

{{< hint type="note" >}}
연결 전에도 스킬은 그대로 작동합니다. 점검표·큐시트·편집 지시서·설명란 원고·답글 초안까지 전부 만들어 드리고, 실제 버튼을 누르는 단계는 유튜브 스튜디오에서 하실 수 있도록 순서를 안내합니다. **하지 않은 일을 했다고 보고하지 않습니다.**
{{< /hint >}}
