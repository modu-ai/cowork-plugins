...
weight: 14
description: "Claude Design 연동, 3계층 토큰 파이프라인, 브랜드 시스템, GAN 품질 루프를 담당하는 디자인 AI 코워커."
aliases: ["/agent-teams/designer/"]
date: 2026-08-07T00:00:00+09:00
lastmod: 2026-08-22T12:48:51+09:00
---

"디자인 감각이 없어서요"라는 말 뒤에는 대개 감각의 문제가 아니라 체계의 부재가 있습니다. 색은 어떤 기준으로 고르고, 버튼 모서리는 왜 그 값이어야 하는지 — 좋은 디자인은 규칙의 집합이고, 그 규칙을 문서로 만든 것이 디자인 시스템입니다. 디자이너 코워커는 이 규칙을 세우고 지키게 하는 코워커입니다. 인테리어로 치면 벽지 한 장을 골라 주는 사람이 아니라, 집 전체의 컬러·자재·조명 기준표를 만들어 어떤 방을 꾸며도 톤이 맞게 해 주는 설계사입니다.

스킬 모음의 중심에는 두 축이 있습니다. 하나는 **Claude Design 연동**(design-\*) — 브리프 작성부터 프롬프트 빌드, 결과물 업로드, 슬롭 체크(AI 특유의 어색한 디자인 패턴 점검)까지 Claude의 디자인 도구와 오가는 파이프라인입니다. 다른 하나는 **토큰 파이프라인** — 디자인 토큰(색·글꼴·간격 같은 디자인 값을 변수로 정리한 것)을 DTCG 표준↔CSS↔Tailwind v4 테마(@theme) 3계층으로 변환해, 디자인 결정이 코드까지 일관되게 흐르게 합니다. 여기에 GAN 품질 루프(생성과 비평을 반복해 완성도를 끌어올리는 반복 개선 방식)가 얹힙니다. 세운 규칙 위에서 실제 이미지를 뽑는 축도 붙었습니다 — Higgsfield 커넥터를 통해 히어로·OG·목업·마스코트 비주얼을 만들되, 만들기 전에 프로젝트의 토큰을 읽어 팔레트와 형태 언어를 제약으로 걸어 둡니다. 여기서 한 걸음 더 나가 **움직이는 랜딩까지** 만듭니다. 스크롤 스크럽·시차 레이어·WebGL 왜곡·파티클·키네틱 타이포 같은 효과를 카탈로그에서 하나만 골라 완성하고, 초기 렌더와 저감모션·모바일 열화를 기계적으로 점검합니다.

{{< hint type="note" >}}
**Claude Design 연동은 Claude Cowork 전용 기능입니다.** `design-workflow`·`design-sync-upload` 스킬이 쓰는 Claude Design 왕복 경로는 Claude Cowork에서만 동작합니다. 다른 스킬(토큰 파이프라인·GAN 루프·Higgsfield 연동)은 양쪽 런타임에서 모두 작동합니다.
{{< /hint >}}

슬래시 명령 없이 자연어로 부릅니다 — "디자인 작업 시작해줘"로 브리프부터 시작하고, "디자인 동기화해서 업로드해줘"로 Claude Design 업로드까지 한 흐름으로 이어집니다.
...
design-\*(Claude Design 연동·시스템·토큰·워크플로우) 계열의 전체 목록입니다.
...
**2. 디자인 토큰을 코드로.** "이 디자인 토큰을 우리 웹사이트 CSS로 변환해줘"라고 요청하면 `design-tokens-transformer`가 DTCG 표준 토큰을 CSS 변수와 Tailwind v4 테마(@theme)로 변환합니다. 디자이너와 개발자 사이의 수작업 전달이 사라집니다.

**3. Claude Design 왕복 작업.** "디자인 작업 시작해줘"로 브리프를 정리해 Claude Design에서 시안을 만들고, 결과를 `design-handoff-reader`로 읽어 와 `design-slop-check`로 어색한 부분을 점검한 뒤 "업로드해줘"로 다시 올리는 왕복 루프를 돕습니다.
...
{{< screenshot-request "Claude Cowork에서 /design 명령 실행 후 브리프 질문이 시작된 대화 화면" >}}
