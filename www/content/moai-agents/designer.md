...
{{< hint type="note" >}}
**Claude Design 연동은 Claude Cowork 전용 기능입니다.** `/design`·`/upload` 명령과 `design-*` 계열 스킬은 Claude Cowork에서만 동작하며, ChatGPT Work에서는 지원하지 않습니다. 다른 스킬(토큰 파이프라인·GAN 루프·Higgsfield 연동)은 양쪽 런타임에서 모두 작동합니다.
{{< /hint >}}
